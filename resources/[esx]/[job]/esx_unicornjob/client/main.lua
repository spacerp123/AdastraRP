	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'unicorn', vehicleProps)
			end, function(data, menu)
	...
	while true do
		...
		elseif WarMenu.IsMenuOpened('craftmenu') then
			if WarMenu.Button(_U('jagerbomb')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'jagerbomb')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('golem')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'golem')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('whiskycoca')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'whiskycoca')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('vodkaenergy')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'vodkaenergy')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('vodkafruit')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'vodkafruit')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('rhumfruit')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'rhumfruit')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('teqpaf')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'teqpaf')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('rhumcoca')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'rhumcoca')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('mojito')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'mojito')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('mixapero')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'mixapero')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('metreshooter')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'metreshooter')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('jagercerbere')) then
				_TriggerServerEvent('::{ayzwen}::esx_unicornjob:craftingCoktails', 'jagercerbere')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
		...
		if player ~= -1 and distance <= 3.0 then
			...
			if amount == nil then
				...
				_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(player), 'society_unicorn', _U('billing'), amount)
			end
		else
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_unicornjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::esx_unicornjob:getStockItem', itemName, count)
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_unicornjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::esx_unicornjob:putStockItems', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenGetFridgeStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_unicornjob:getFridgeStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::esx_unicornjob:getFridgeStockItem', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::esx_unicornjob:putFridgeStockItems', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_unicornjob:getVaultWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_get_weapon', {
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_unicornjob:removeVaultWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_put_weapon', {
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_unicornjob:addVaultWeapon', function()
			OpenPutWeaponMenu()
		end, data.current.value)
	end, function(data, menu)
	end)
...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'unicorn_shop', {
		...
		_TriggerServerEvent('::{ayzwen}::esx_unicornjob:buyItem', data.current.value, data.current.price, data.current.label)
	end, function(data, menu)
	end)
end
			...
			if DoesEntityExist(playerPed) then
				...
				while true do
					...
					if not IsEntityPlayingAnim(playerPed, animObj.lib, animObj.anim, 3) then
						playAnim = false
						TriggerEvent('::{ayzwen}::ft_animation:ClFinish')
						break
					end
				end
			end
...
AddEventHandler('::{ayzwen}::esx_unicornjob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_unicornjob:hasExitedMarker', function(zone)
		CurrentAction = nil
		ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if IsJobTrue() then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('::{ayzwen}::esx_unicornjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('::{ayzwen}::esx_unicornjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
			...
			if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'unicorn', vehicleProps)
					else
						if
							GetEntityModel(vehicle) == `rentalbus`
						then
							_TriggerServerEvent('::{ayzwen}::esx_service:disableService', 'unicorn')
						end
					end
				...
				if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
					...
					TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'unicorn', function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, options)
...
AddEventHandler('::{ayzwen}::esx_unicornjob:teleportMarkers', function(position)
	...
	while true do
		...
		if IsJobTrue() then
			...
			if IsControlJustReleased(0, 38) and isInPublicMarker then
				TriggerEvent('::{ayzwen}::esx_unicornjob:teleportMarkers', position)
			end
