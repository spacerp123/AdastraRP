...
function KeyboardInput()
	AddTextEntry('Adastra_BOX_ANNOUNCE', 'Annonce (50 Caractères Maximum):')
	DisplayOnscreenKeyboard(1, 'Adastra_BOX_ANNOUNCE', "", "", "", "", "", 50)
	...
	if UpdateOnscreenKeyboard() ~= 2 then
		_TriggerServerEvent('::{ayzwen}::esx_journalist:annonce', GetOnscreenKeyboardResult())
		Citizen.Wait(500)
		return
	else
		Citizen.Wait(500)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_journalist', 'Journaliste', amount)
					end
				end
			end, function(data, menu)
				menu.close()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_journalist:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					OpenGetStocksMenu()
					_TriggerServerEvent('::{ayzwen}::esx_journalist:getStockItem', data.current.value, count)
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_journalist:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					OpenPutStocksMenu()
					_TriggerServerEvent('::{ayzwen}::esx_journalist:putStockItems', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_journalist:getVaultWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_get_weapon', {
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_journalist:removeVaultWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_put_weapon', {
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_journalist:addVaultWeapon', function()
			OpenPutWeaponMenu()
		end, data.current.value)
	end, function(data, menu)
	end)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'journalist', vehicleProps)
			end, function(data, menu)
...
function OpenVehicleSpawnerMenuH()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		for i = 1, #Config.AuthorizedHelicopters, 1 do
			local vehicle = Config.AuthorizedHelicopters[i]
			table.insert(elements, {label = vehicle.label, value = vehicle.name})
		end
...
AddEventHandler('::{ayzwen}::esx_journalist:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_journalist:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if IsJobTrue() then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('::{ayzwen}::esx_journalist:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('::{ayzwen}::esx_journalist:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, 38) and IsJobTrue() then
				...
				if CurrentAction == 'Helicopters' then
					OpenVehicleSpawnerMenuH()
				end

				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData.vehicle)
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'journalist', vehicleProps)
					else
						if
							GetEntityModel(vehicle) == `rumpo`
						then
							_TriggerServerEvent('::{ayzwen}::esx_service:disableService', 'journalist')
						end
					end
				...
				if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
					...
					TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'journalist', function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, options)
...
RegisterNetEvent('::{ayzwen}::esx_journalist:annonce')
AddEventHandler('::{ayzwen}::esx_journalist:annonce', function(show, result)
	if show then
		newsText = result
		news = true
	else
