...
function OpenVigneActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vigne_actions', {
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'vigne', function(data, menu) end)
		end
	end, function(data, menu)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'vigne', vehicleProps)
			end, function(data, menu)
				...
				if amount == nil or amount <= 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						Citizen.CreateThread(function()
							...
							_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_vigne', 'Vigneron', amount)
						end)
					end
				end
			end, function(data, menu)
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_vigneronjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil or count <= 0 then
					...
					_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:getStockItem', itemName, count)
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_vigneronjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil or count <= 0 then
					...
					_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:putStockItems', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
AddEventHandler('::{ayzwen}::esx_vigneronjob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_vigneronjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'RaisinFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:stopHarvest')
	end  
	if (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:stopSell')
	end
	CurrentAction = nil
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('::{ayzwen}::esx_vigneronjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('::{ayzwen}::esx_vigneronjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					_TriggerServerEvent('::{ayzwen}::esx_vigneronjob:startSell', CurrentActionData.zone)
				end
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'vigne', vehicleProps)
						_TriggerServerEvent('::{ayzwen}::esx_vehiclelock:deletekeyjobs', 'no', plate) --vehicle lock
					end
