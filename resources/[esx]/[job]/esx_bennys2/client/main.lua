...
AddEventHandler('::{ayzwen}::esx:playerLoaded', function(xPlayer)
  	PlayerData = xPlayer
	ESX.TriggerServerCallback('::{ayzwen}::esx_lscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('::{ayzwen}::esx_lscustom:installMod')
AddEventHandler('::{ayzwen}::esx_lscustom:installMod', function()
	myCar = ESX.Game.GetVehicleProperties(GetVehiclePedIsIn(PlayerPedId(), false))
	_TriggerServerEvent('::{ayzwen}::esx_lscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('::{ayzwen}::esx_lscustom:cancelInstallMod')
AddEventHandler('::{ayzwen}::esx_lscustom:cancelInstallMod', function()
...
function OpenLSMenu(elems, menuname, menutitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuname, {
		...
		for k, v in pairs(Config.Menus) do
			if k == data.current.modType or isRimMod then
				if data.current.label == _U('by_default') or string.match(data.current.rightlabel[1], _U('installed')) then
					...
					TriggerEvent('::{ayzwen}::esx_lscustom:installMod')
				else
					...
					if isRimMod then
						...
						_TriggerServerEvent("::{ayzwen}::esx_lscustom:buyMod", price)
		...
		if isBuying == nil then
			TriggerEvent('::{ayzwen}::esx_lscustom:cancelInstallMod')
		else
			isBuying = nil
		end
