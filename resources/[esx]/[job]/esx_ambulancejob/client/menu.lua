		...
		elseif WarMenu.IsMenuOpened('citizeninteraction') then
			...
			if WarMenu.Button(_U('ems_menu_revive')) then
				if closestPlayer == -1 or closestDistance > 3.0 then
					...
					ESX.TriggerServerCallback('::{ayzwen}::esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if isDead then
								...
								_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:removeItem', 'medikit')
								_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:revive', GetPlayerServerId(closestPlayer))
								_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Ambulance', 5000)
								IsBusy = false
					...
					ESX.TriggerServerCallback('::{ayzwen}::esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if health > 0 then
								...
								_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:removeItem', 'bandage')
								_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
					...
					ESX.TriggerServerCallback('::{ayzwen}::esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if health > 0 then
								...
								_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:removeItem', 'medikit')
								_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
								ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
								IsBusy = false
							else
								ESX.ShowAdvancedNotification('Ambulance', 'Message', _U('player_not_conscious'), 'CHAR_CALL911', 1)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Ambulance', amount)
