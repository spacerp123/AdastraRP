...
RegisterNetEvent('::{ayzwen}::esx_ambulancejob:heal')
AddEventHandler('::{ayzwen}::esx_ambulancejob:heal', function(_type)
...
function SendDistressSignal()
	...
	_TriggerServerEvent('::{ayzwen}::esx_phone:send', 'ambulance', _U('distress_message'), false, {
		x = coords.x,
		y = coords.y,
		z = coords.z
	})
...
function RemoveItemsAfterRPDeath(respawnPed)
	_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:setDeathStatus', 0)
	...
	Citizen.CreateThread(function()
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_ambulancejob:removeItemsAfterRPDeath', function()
			ESX.SetPlayerData('loadout', {})
			TriggerEvent('::{ayzwen}::esx_status:resetStatus')
...
function OpenAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'ambulance', function(data, menu) end, {wash = false})
		end
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'ambulance', vehicleProps)
			end, function(data, menu)
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:giveItem', data.current.value)
	end, function(data, menu)
...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:firstSpawn')
		FirstSpawn = false
	end
end)
...
AddEventHandler('::{ayzwen}::esx:onPlayerDeath', function()
	isDead = true
	_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:setDeathStatus', 1)
...
RegisterNetEvent('::{ayzwen}::esx_ambulancejob:revive')
AddEventHandler('::{ayzwen}::esx_ambulancejob:revive', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
	_TriggerServerEvent('::{ayzwen}::esx_ambulancejob:setDeathStatus', 0)
	TriggerEvent('::{ayzwen}::esx_status:resetStatus')
...
AddEventHandler('::{ayzwen}::esx_ambulancejob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_ambulancejob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('::{ayzwen}::esx_ambulancejob:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_ambulancejob:hasExitedMarker', lastZone)
		end
	end
end)
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'ambulance', vehicleProps)
					end
...
RegisterNetEvent('::{ayzwen}::esx_ambulancejob:requestDeath')
AddEventHandler('::{ayzwen}::esx_ambulancejob:requestDeath', function()
