	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'police', data.current.value)
			end, function(data, menu)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(player), 'society_police', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if (data.value == nil) or (string.len(data.value) ~= 8) then
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			_TriggerServerEvent('::{ayzwen}::esx_policejob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:getVehicleInfos', function(retrivedInfo)
		local elements = {}
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{ayzwen}::esx_policejob:getStockItem', data.current.value, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_policejob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{ayzwen}::esx_policejob:putStockItems', itemName, count)
...
AddEventHandler('::{ayzwen}::esx_policejob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('::{ayzwen}::esx_policejob:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('::{ayzwen}::esx_policejob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('::{ayzwen}::esx_policejob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('::{ayzwen}::esx_policejob:drag')
AddEventHandler('::{ayzwen}::esx_policejob:drag', function(draggerId)
...
RegisterNetEvent('::{ayzwen}::esx_policejob:putInVehicle')
AddEventHandler('::{ayzwen}::esx_policejob:putInVehicle', function()
	local plyPed = PlayerPedId()
...
RegisterNetEvent('::{ayzwen}::esx_policejob:OutVehicle')
AddEventHandler('::{ayzwen}::esx_policejob:OutVehicle', function()
	local plyPed = PlayerPedId()
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				if (LastStation ~= nil and LastPart ~= nil and LastPartNum ~= nil) and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) then
					TriggerEvent('::{ayzwen}::esx_policejob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('::{ayzwen}::esx_policejob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('::{ayzwen}::esx_policejob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('::{ayzwen}::esx_policejob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
			...
			if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'police', vehicleProps)
					end
					ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'police', function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, {wash = false})
...
AddEventHandler('playerSpawned', function()
	isDead = false
	TriggerEvent('::{ayzwen}::esx_policejob:unrestrain')

	if not hasAlreadyJoined then
		_TriggerServerEvent('::{ayzwen}::esx_policejob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('::{ayzwen}::esx_policejob:unrestrain')
		TriggerEvent('::{ayzwen}::esx_phone:removeSpecialContact', 'police')

		if Config.MaxInService ~= -1 then
			_TriggerServerEvent('::{ayzwen}::esx_service:disableService', 'police')
		end
	end
end)
