...
--table.insert(elements, {label = 'Enlever le Gilet Cadet', value = 'gilet_wear1'})
	...
	table.insert(elements, {label = 'Tenue Civil', value = 'citizen_wear'})
...
function OpenVehicleSpawnerMenu(station, partNum)
	local vehicles = Config.GouvernementStations[station].Vehicles
	ESX.UI.Menu.CloseAll()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'gouvernement', data.current.value)
			end, function(data, menu)
			...
			if not DoesEntityExist(vehicle) then
				...
				if Config.MaxInService == -1 then
					ESX.Game.SpawnVehicle(model, vehicles[partNum].SpawnPoint, vehicles[partNum].Heading, function(vehicle)
						local newPlate = exports['esx_vehicleshop']:GenerateSocietyPlate('GOUV')
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(player), 'society_gouvernement', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if (data.value == nil) or (string.len(data.value) ~= 8) then
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			_TriggerServerEvent('::{ayzwen}::esx_gouvjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:getVehicleInfos', function(retrivedInfo)
		local elements = {}
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{ayzwen}::esx_gouvjob:getStockItem', data.current.value, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_gouvjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{ayzwen}::esx_gouvjob:putStockItems', itemName, count)
...
AddEventHandler('::{ayzwen}::esx_gouvjob:hasEnteredMarker', function(station, part, partNum)
  ...
  if part == 'HelicopterSpawner' then
    local helicopters = Config.GouvernementStations[station].Helicopters
...
AddEventHandler('::{ayzwen}::esx_gouvjob:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('::{ayzwen}::esx_gouvjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('::{ayzwen}::esx_gouvjob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('::{ayzwen}::esx_gouvjob:drag')
AddEventHandler('::{ayzwen}::esx_gouvjob:drag', function(draggerId)
...
RegisterNetEvent('::{ayzwen}::esx_gouvjob:putInVehicle')
AddEventHandler('::{ayzwen}::esx_gouvjob:putInVehicle', function()
	local plyPed = PlayerPedId()
...
RegisterNetEvent('::{ayzwen}::esx_gouvjob:OutVehicle')
AddEventHandler('::{ayzwen}::esx_gouvjob:OutVehicle', function()
	local plyPed = PlayerPedId()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement' then
			...
			for k, v in pairs(Config.GouvernementStations) do
				...
				if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement' and PlayerData.job.grade_name == 'boss' then
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				if (LastStation ~= nil and LastPart ~= nil and LastPartNum ~= nil) and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) then
					TriggerEvent('::{ayzwen}::esx_gouvjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('::{ayzwen}::esx_gouvjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('::{ayzwen}::esx_gouvjob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('::{ayzwen}::esx_gouvjob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'gouvernement', vehicleProps)
					end
					ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'gouvernement', function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, {wash = false})
		...
		if (IsControlJustReleased(0, 167) or IsDisabledControlJustReleased(0, 167)) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'gouvernement' then
			if Config.MaxInService == -1 then
				WarMenu.OpenMenu('gouvernementmenu')
			elseif playerInService then
				WarMenu.OpenMenu('gouvernementmenu')
			else
				ESX.ShowNotification(_U('service_not'))
...
AddEventHandler('playerSpawned', function()
	isDead = false
	TriggerEvent('::{ayzwen}::esx_gouvjob:unrestrain')

	if not hasAlreadyJoined then
		_TriggerServerEvent('::{ayzwen}::esx_gouvjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('::{ayzwen}::esx_gouvjob:unrestrain')
		TriggerEvent('::{ayzwen}::esx_phone:removeSpecialContact', 'gouvernement')

		if Config.MaxInService ~= -1 then
			_TriggerServerEvent('::{ayzwen}::esx_service:disableService', 'gouvernement')
		end
	end
end)
