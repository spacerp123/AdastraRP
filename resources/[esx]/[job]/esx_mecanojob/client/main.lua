...
function OpenMecanoActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecano_actions', {
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'mecano', vehicleProps)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'mecano', function(data, menu) end)
		end
	end, function(data, menu)
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecano_harvest', {
			...
			if data.current.value == 'gaz_bottle' then
				menu.close()
				_TriggerServerEvent('::{ayzwen}::esx_mecanojob:startHarvest')
			elseif data.current.value == 'fix_tool' then
				menu.close()
				_TriggerServerEvent('::{ayzwen}::esx_mecanojob:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				menu.close()
				_TriggerServerEvent('::{ayzwen}::esx_mecanojob:startHarvest3')
			end
		end, function(data, menu)
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecano_craft', {
			...
			if data.current.value == 'blow_pipe' then
				menu.close()
				_TriggerServerEvent('::{ayzwen}::esx_mecanojob:startCraft')
			elseif data.current.value == 'fix_kit' then
				menu.close()
				_TriggerServerEvent('::{ayzwen}::esx_mecanojob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				menu.close()
				_TriggerServerEvent('::{ayzwen}::esx_mecanojob:startCraft3')
			end
		end, function(data, menu)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
					...
					if var == nil or var < 0 then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							menu.close()
							_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mecano', _U('mechanic'), var)
						end
					end
				end, function(data, menu)
					menu.close()
						...
						if NPCOnJob then
							if NPCTargetDeleterZone then
								...
								if CurrentlyTowedVehicle == NPCTargetTowable then
									...
									_TriggerServerEvent('::{ayzwen}::esx_mecanojob:onNPCJobMissionCompleted')
									StopNPCJob()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_mecanojob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{ayzwen}::esx_mecanojob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_mecanojob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{ayzwen}::esx_mecanojob:putStockItems', itemName, count)
...
RegisterNetEvent('::{ayzwen}::esx_mecanojob:onHijack')
AddEventHandler('::{ayzwen}::esx_mecanojob:onHijack', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
...
RegisterNetEvent('::{ayzwen}::esx_mecanojob:onCarokit')
AddEventHandler('::{ayzwen}::esx_mecanojob:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
...
RegisterNetEvent('::{ayzwen}::esx_mecanojob:onFixkit')
AddEventHandler('::{ayzwen}::esx_mecanojob:onFixkit', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
...
AddEventHandler('::{ayzwen}::esx_mecanojob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_mecanojob:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		_TriggerServerEvent('::{ayzwen}::esx_mecanojob:stopCraft')
		_TriggerServerEvent('::{ayzwen}::esx_mecanojob:stopCraft2')
		_TriggerServerEvent('::{ayzwen}::esx_mecanojob:stopCraft3')
	elseif zone == 'Garage' then
		_TriggerServerEvent('::{ayzwen}::esx_mecanojob:stopHarvest')
		_TriggerServerEvent('::{ayzwen}::esx_mecanojob:stopHarvest2')
		_TriggerServerEvent('::{ayzwen}::esx_mecanojob:stopHarvest3')
	end
...
AddEventHandler('::{ayzwen}::esx_mecanojob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('::{ayzwen}::esx_mecanojob:hasExitedEntityZone', function(entity)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('::{ayzwen}::esx_mecanojob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('::{ayzwen}::esx_mecanojob:hasExitedMarker', LastZone)
			end
		end
	end
end)
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('::{ayzwen}::esx_mecanojob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('::{ayzwen}::esx_mecanojob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
			...
			if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'mecano', vehicleProps)
					else
						if
						...
						then
							_TriggerServerEvent('::{ayzwen}::esx_service:disableService', 'mecano')
						end
					end
