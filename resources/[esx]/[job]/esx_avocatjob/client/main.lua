...
function OpenAvocatActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'avocat_actions', {
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('::{ayzwen}::esx_society:openBossMenu', 'avocat', function(data, menu) end)
		end
	end, function(data, menu)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', 'avocat', vehicleProps)
			end, function(data, menu)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount <= 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_avocat', 'Avocat', amount)
					end
				end
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_avocatjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil or count <= 0 then
					...
					menu.close()
					OpenGetStocksMenu()
					_TriggerServerEvent('::{ayzwen}::esx_avocatjob:getStockItem', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_avocatjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil or count <= 0 then
					...
					menu.close()
					OpenPutStocksMenu()
					_TriggerServerEvent('::{ayzwen}::esx_avocatjob:putStockItems', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
AddEventHandler('::{ayzwen}::esx_avocatjob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_avocatjob:hasExitedMarker', function(zone)
	CurrentAction = nil
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'avocat' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('::{ayzwen}::esx_avocatjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('::{ayzwen}::esx_avocatjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
			...
			if IsControlPressed(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'avocat' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', 'avocat', vehicleProps)
