...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		if data.current.value == 'clip' then
			_TriggerServerEvent('::{ayzwen}::esx_clipshop:buyChargeur', zone)
		end
...
AddEventHandler('::{ayzwen}::esx_clipshop:hasEnteredMarker', function(zone)
	CurrentAction = 'shop_menu'
	CurrentActionMsg = _U('shop_menu')
	CurrentActionData = {zone = zone}
end)

AddEventHandler('::{ayzwen}::esx_clipshop:hasExitedMarker', function(zone)
	CurrentAction = nil
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('::{ayzwen}::esx_clipshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_clipshop:hasExitedMarker', LastZone)
		end
	end
end)
