...
RegisterNetEvent('::{ayzwen}::esx_armour:armor')
AddEventHandler('::{ayzwen}::esx_armour:armor', function()
	...
	if GetPedArmour(plyPed) == 100 then
		...
		_TriggerServerEvent('::{ayzwen}::esx_armour:armorremove')

		TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
			if skin.sex == 0 then
				TriggerEvent('::{ayzwen}::skinchanger:loadClothes', skin, {['bproof_1'] = 11,  ['bproof_2'] = 1})
				SetPedArmour(PlayerPedId(), 100)
			else
				TriggerEvent('::{ayzwen}::skinchanger:loadClothes', skin, {['bproof_1'] = 13,  ['bproof_2'] = 1})
				SetPedArmour(PlayerPedId(), 100)
			end
		end)
...
RegisterNetEvent('::{ayzwen}::esx_armour:handcuff')
AddEventHandler('::{ayzwen}::esx_armour:handcuff', function()
	...
	Citizen.CreateThread(function()
		...
		if distance ~= -1 and distance <= 3.0 then
			if IsHandcuffed then
				...
				_TriggerServerEvent('::{ayzwen}::esx_armour:handcuffremove')
...
RegisterNetEvent('::{ayzwen}::esx_armour:cutting_pliers')
AddEventHandler('::{ayzwen}::esx_armour:cutting_pliers', function()
	IsHandcuffed = not IsHandcuffed
	local playerPed = PlayerPedId()
