		...
		for i = 1, #atms, 1 do
			if #(atms[i] - plyCoords) <= 2.0 then
				...
				if IsControlJustReleased(0, 38) then
					...
					_TriggerServerEvent('::{ayzwen}::bank:balance')
				end
			end
		end
...
RegisterNetEvent('::{ayzwen}::bank:refreshbalance')
AddEventHandler('::{ayzwen}::bank:refreshbalance', function(balance)
	SendNUIMessage({
		type = 'balanceHUD',
		balance = balance,
...
RegisterNUICallback('deposit', function(data)
	_TriggerServerEvent('::{ayzwen}::bank:deposit', tonumber(data.amount))
	_TriggerServerEvent('::{ayzwen}::bank:balance')
end)

RegisterNUICallback('withdrawl', function(data)
	_TriggerServerEvent('::{ayzwen}::bank:withdraw', tonumber(data.amountw))
	_TriggerServerEvent('::{ayzwen}::bank:balance')
end)
...
RegisterNetEvent('::{ayzwen}::balance:back')
AddEventHandler('::{ayzwen}::balance:back', function(balance)
	SendNUIMessage({type = 'balanceReturn', bal = balance})
end)

RegisterNUICallback('transfer', function(data)
	_TriggerServerEvent('::{ayzwen}::bank:transfer', data.to, data.amountt)
	_TriggerServerEvent('::{ayzwen}::bank:balance')
end)

RegisterNetEvent('::{ayzwen}::bank:result')
AddEventHandler('::{ayzwen}::bank:result', function(type, message)
