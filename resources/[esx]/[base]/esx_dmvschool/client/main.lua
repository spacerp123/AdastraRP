...
function StartTheoryTest()
	...
	_TriggerServerEvent('::{ayzwen}::esx_dmvschool:pay', Config.Prices['dmv'])
...
function StopTheoryTest(success)
	...
	if success then
		_TriggerServerEvent('::{ayzwen}::esx_dmvschool:addLicense', 'dmv')
...
function StartDriveTest(type)
	...
	_TriggerServerEvent('::{ayzwen}::esx_dmvschool:pay', Config.Prices[type])
end

function StopDriveTest(success)
	if success then
		_TriggerServerEvent('::{ayzwen}::esx_dmvschool:addLicense', CurrentTestType)
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
...
AddEventHandler('::{ayzwen}::esx_dmvschool:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_dmvschool:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('::{ayzwen}::esx_dmvschool:loadLicenses')
AddEventHandler('::{ayzwen}::esx_dmvschool:loadLicenses', function(licenses)
	Licenses = licenses
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('::{ayzwen}::esx_dmvschool:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_dmvschool:hasExitedMarker', LastZone)
		end
	end
end)
