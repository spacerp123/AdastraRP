...
AddEventHandler('::{ayzwen}::esx:playerLoaded', function(xPlayer)
	ESX.TriggerServerCallback('::{ayzwen}::esx_property:getProperties', function(properties)
		Config.Properties = properties
		CreateBlips()
	end)

	ESX.TriggerServerCallback('::{ayzwen}::esx_property:getOwnedProperties', function(ownedProperties)
...
RegisterNetEvent('::{ayzwen}::esx_property:sendProperties')
AddEventHandler('::{ayzwen}::esx_property:sendProperties', function(properties)
	Config.Properties = properties
	CreateBlips()
...
function EnterProperty(name, owner)
	...
	_TriggerServerEvent('::{ayzwen}::esx_property:saveLastProperty', name)
...
function ExitProperty(name)
	...
	_TriggerServerEvent('::{ayzwen}::esx_property:deleteLastProperty')
...
function OpenPropertyMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		if data.current.value == 'enter' then
			TriggerEvent('::{ayzwen}::instance:create', 'property', {property = property.name, owner = ESX.GetPlayerData().identifier})
		elseif data.current.value == 'leave' then
			_TriggerServerEvent('::{ayzwen}::esx_property:removeOwnedProperty', property.name)
		elseif data.current.value == 'buy' then
			_TriggerServerEvent('::{ayzwen}::esx_property:buyProperty', property.name)
		elseif data.current.value == 'rent' then
			_TriggerServerEvent('::{ayzwen}::esx_property:rentProperty', property.name)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties_actions', {
			...
			if data2.current.value == 'enter' then
				TriggerEvent('::{ayzwen}::instance:create', 'property', {property = data.current.value, owner = ESX.GetPlayerData().identifier})
				ESX.UI.Menu.CloseAll()
			elseif data2.current.value == 'leave' then
				_TriggerServerEvent('::{ayzwen}::esx_property:removeOwnedProperty', data.current.value)
			end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			...
			if data2.current.value == 'buy' then
				_TriggerServerEvent('::{ayzwen}::esx_property:buyProperty', data.current.value)
			elseif data2.current.value == 'rent' then
				_TriggerServerEvent('::{ayzwen}::esx_property:rentProperty', data.current.value)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		if data.current.value == 'invite_player' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_invite', {
				...
				TriggerEvent('::{ayzwen}::instance:invite', 'property', GetPlayerServerId(data2.current.value), {property = property.name, owner = owner})
				ESX.ShowNotification(_U('you_invited', GetPlayerName(data2.current.value)))
			end, function(data2, menu2)
			end)
		elseif data.current.value == 'player_dressing' then
			ESX.TriggerServerCallback('::{ayzwen}::esx_property:getPlayerDressing', function(dressing)
				local elements = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					...
					TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
						ESX.TriggerServerCallback('::{ayzwen}::esx_property:getPlayerOutfit', function(clothes)
							TriggerEvent('::{ayzwen}::skinchanger:loadClothes', skin, clothes)
							TriggerEvent('::{ayzwen}::esx_skin:setLastSkin', skin)
							TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
								_TriggerServerEvent('::{ayzwen}::esx_skin:save', skin)
							end)
		...
		elseif data.current.value == 'remove_cloth' then
			ESX.TriggerServerCallback('::{ayzwen}::esx_property:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					menu2.close()
					_TriggerServerEvent('::{ayzwen}::esx_property:removeOutfit', data2.current.value)
...
function OpenRoomInventoryMenu(property, owner)
	ESX.TriggerServerCallback('::{ayzwen}::esx_property:getPropertyInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				_TriggerServerEvent('::{ayzwen}::esx_property:getItem', owner, data.current.type, data.current.value, data.current.ammo)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					...
					if quantity == nil then
						...
						_TriggerServerEvent('::{ayzwen}::esx_property:getItem', owner, data.current.type, data.current.value, quantity)
						ESX.SetTimeout(300, function()
							OpenRoomInventoryMenu(property, owner)
						end)
					end
...
function OpenPlayerInventoryMenu(property, owner)
	ESX.TriggerServerCallback('::{ayzwen}::esx_property:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				_TriggerServerEvent('::{ayzwen}::esx_property:putItem', owner, data.current.type, data.current.value, data.current.ammo)
				ESX.SetTimeout(300, function()
					OpenPlayerInventoryMenu(property, owner)
				end)
			else
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
					...
					if quantity == nil then
						...
						_TriggerServerEvent('::{ayzwen}::esx_property:putItem', owner, data.current.type, data.current.value, tonumber(data2.value))
						ESX.SetTimeout(300, function()
							OpenPlayerInventoryMenu(property, owner)
						end)
					end
...
AddEventHandler('::{ayzwen}::instance:loaded', function()
	TriggerEvent('::{ayzwen}::instance:registerType', 'property', function(instance)
...
AddEventHandler('playerSpawned', function()
	if firstSpawn then
		Citizen.CreateThread(function()
			...
			ESX.TriggerServerCallback('::{ayzwen}::esx_property:getLastProperty', function(propertyName)
				if propertyName then
					if propertyName ~= '' then
						...
						TriggerEvent('::{ayzwen}::instance:create', 'property', {property = propertyName, owner = ESX.GetPlayerData().identifier})
					end
				end
			end)
		end)
...
AddEventHandler('::{ayzwen}::esx_property:getProperties', function(cb)
	cb(GetProperties())
end)

AddEventHandler('::{ayzwen}::esx_property:getProperty', function(name, cb)
	cb(GetProperty(name))
end)

AddEventHandler('::{ayzwen}::esx_property:getGateway', function(property, cb)
	cb(GetGateway(property))
end)

RegisterNetEvent('::{ayzwen}::esx_property:setPropertyOwned')
AddEventHandler('::{ayzwen}::esx_property:setPropertyOwned', function(name, owned)
	SetPropertyOwned(name, owned)
end)

RegisterNetEvent('::{ayzwen}::instance:onCreate')
AddEventHandler('::{ayzwen}::instance:onCreate', function(instance)
	if instance.type == 'property' then
		TriggerEvent('::{ayzwen}::instance:enter', instance)
	end
end)

RegisterNetEvent('::{ayzwen}::instance:onEnter')
AddEventHandler('::{ayzwen}::instance:onEnter', function(instance)
	if instance.type == 'property' then
...
RegisterNetEvent('::{ayzwen}::instance:onPlayerLeft')
AddEventHandler('::{ayzwen}::instance:onPlayerLeft', function(instance, player)
	if player == instance.host then
		TriggerEvent('::{ayzwen}::instance:leave')
	end
end)

AddEventHandler('::{ayzwen}::esx_property:hasEnteredMarker', function(name, part)
	local property = GetProperty(name)
...
AddEventHandler('::{ayzwen}::esx_property:hasExitedMarker', function(name, part)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker or (isInMarker and (LastProperty ~= currentProperty or LastPart ~= currentPart) ) then
			...
			TriggerEvent('::{ayzwen}::esx_property:hasEnteredMarker', currentProperty, currentPart)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_property:hasExitedMarker', LastProperty, LastPart)
		end
