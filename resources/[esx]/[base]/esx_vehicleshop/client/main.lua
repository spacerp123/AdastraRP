...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
...
RegisterNetEvent('::{ayzwen}::esx_vehicleshop:sendCategories')
AddEventHandler('::{ayzwen}::esx_vehicleshop:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('::{ayzwen}::esx_vehicleshop:sendVehicles')
AddEventHandler('::{ayzwen}::esx_vehicleshop:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getCommercialVehicles', function(vehicles)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:returnProvider', data.current.value)
			Citizen.Wait(300)
			menu.close()
	...
	for k, v in ipairs(Categories) do
		if v.society == LastZone[1] then
			...
			for k2, v2 in ipairs(vehiclesByCategory[v.name]) do
				...
				table.insert(options.description, "~s~Prix d\'achat: ~r~"..ESX.Math.GroupDigits(v2.price).."$\n~s~Prix de vente: ~g~"..ESX.Math.GroupDigits(v2.price * 1.5).."$")
			end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:buyVehicleSociety', function(hasEnoughMoney)
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:buyVehicle', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones[LastZone[1]].ShopOutside.Pos, Config.Zones[LastZone[1]].ShopOutside.Heading, function(vehicle)
											...
											if Config.EnableOwnedVehicles then
												_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:setVehicleOwned', vehicleProps, getVehicleType(vehicleProps.model))
											end
								...
								ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:buyVehicleSociety', function(hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones[LastZone[1]].ShopOutside.Pos, Config.Zones[LastZone[1]].ShopOutside.Heading, function(vehicle)
											...
											_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:setVehicleOwnedSociety', playerData.job.name, vehicleProps, getVehicleType(vehicleProps.model))
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{ayzwen}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_' .. ESX.PlayerData.job.name, _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function(data2, menu2)
				menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:sellVehicle', model)
				_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:addToList', GetPlayerServerId(closestPlayer), model, newPlate, LastZone[1])

				if Config.EnableOwnedVehicles then
					_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, getVehicleType(vehicleProps.model))
		...
		elseif action == 'set_vehicle_owner_sell_society' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('::{ayzwen}::esx:getOtherPlayerData', function(xPlayer)
					local newPlate = GeneratePlate()
					local vehicleProps = ESX.Game.GetVehicleProperties(LastVehicles[#LastVehicles])
					local model = CurrentVehicleData.model
					vehicleProps.plate = newPlate
					...
					if Config.EnableSocietyOwnedVehicles then
						_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps, getVehicleType(vehicleProps.model))
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(LastVehicles[#LastVehicles], newPlate)
						_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:rentVehicle', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer), LastZone[1])
						...
						_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:setVehicleForAllPlayers', vehicleProps, Config.Zones[LastZone[1]].ShopInside.Pos, 5.0)
					end
				end
			end, function(data2, menu2)
				menu2.close()
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getRentedVehicles', function(vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('::{ayzwen}::esx_society:openBossMenu', LastZone[1], function(data, menu) end)
		elseif data.current.value == 'sold_vehicles' then
			ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:getStockItem', LastZone[1], itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::esx_vehicleshop:putStockItems', LastZone[1], itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('::{ayzwen}::esx_vehicleshop:hasEnteredMarker', function(zone)
...
AddEventHandler('::{ayzwen}::esx_vehicleshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and (LastZone[1] ~= currentZone[1] or LastZone[2] ~= currentZone[2])) then
			...
			TriggerEvent('::{ayzwen}::esx_vehicleshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_vehicleshop:hasExitedMarker', LastZone)
		end
	end
end)
			...
			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'shop_menu' then
					if Config.LicenseEnable then
						ESX.TriggerServerCallback('::{ayzwen}::esx_license:checkLicense', function(hasDriversLicense)
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'carshop' and ESX.PlayerData.job.grade_name == 'boss' then
					--[[
					ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
