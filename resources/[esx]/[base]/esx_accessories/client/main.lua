...
function OpenShopMenu(accessory)
	...
	TriggerEvent('::{ayzwen}::esx_skin:openRestrictedMenu', function(data, menu)
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				ESX.TriggerServerCallback('::{ayzwen}::esx_accessories:checkMoney', function(hasEnoughMoney)
					if hasEnoughMoney then
						_TriggerServerEvent('::{ayzwen}::esx_accessories:pay')
						TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
							_TriggerServerEvent('::{ayzwen}::esx_accessories:save', skin, accessory)
						end)
					else
...
AddEventHandler('::{ayzwen}::esx_accessories:hasEnteredMarker', function(zone)
	CurrentAction = 'shop_menu'
	CurrentActionMsg = _U('press_access')
	CurrentActionData = { accessory = zone }
end)

AddEventHandler('::{ayzwen}::esx_accessories:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('::{ayzwen}::esx_accessories:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_accessories:hasExitedMarker', LastZone)
		end
	end
end)
