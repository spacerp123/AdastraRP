	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Tattoos_menu', {
		...
		if (data.current.value ~= nil) then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Tattoos_Categories_menu', {
				...
				_TriggerServerEvent("::{ayzwen}::tattoos:save", currentTattoos, data2.current.price, {collection = data.current.value, texture = data2.current.value})
...
RegisterNetEvent("::{ayzwen}::tattoos:getPlayerTattoos")
AddEventHandler("::{ayzwen}::tattoos:getPlayerTattoos", function(playerTattoosList)
...
AddEventHandler("::{ayzwen}::skinchanger:loadSkin", function(skin)
	if not firstLoad then
		Citizen.CreateThread(function()
			...
			Citizen.Wait(75)
			_TriggerServerEvent("::{ayzwen}::tattoos:GetPlayerTattoos_s")
		end)
...
RegisterNetEvent("::{ayzwen}::tattoo:buySuccess")
AddEventHandler("::{ayzwen}::tattoo:buySuccess", function(value)
