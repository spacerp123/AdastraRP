...
RegisterNetEvent('::{ayzwen}::esx_identity:showRegisterIdentity')
AddEventHandler('::{ayzwen}::esx_identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)

RegisterNetEvent('::{ayzwen}::esx_identity:identityCheck')
AddEventHandler('::{ayzwen}::esx_identity:identityCheck', function(identityCheck)
...
RegisterNUICallback('register', function(myIdentity, cb)
	...
	if reason == nil then
		_TriggerServerEvent('::{ayzwen}::esx_identity:setIdentity', myIdentity)
		EnableGui(false)
		Citizen.Wait(500)
