...
RegisterNetEvent('::{ayzwen}::esx_kekke_tackle:getTackled')
AddEventHandler('::{ayzwen}::esx_kekke_tackle:getTackled', function(targetId)
...
RegisterNetEvent('::{ayzwen}::esx_kekke_tackle:playTackle')
AddEventHandler('::{ayzwen}::esx_kekke_tackle:playTackle', function()
	local plyPed = PlayerPedId()
	...
	while true do
		...
		if IsControlPressed(0, 21) and IsControlPressed(0, 74) and not isTackling and GetGameTimer() - lastTackleTime > 5 * 1000 then
			...
			if distance ~= -1 and distance <= Config.TackleDistance and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(PlayerPedId()) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				_TriggerServerEvent('::{ayzwen}::esx_kekke_tackle:tryTackle', GetPlayerServerId(closestPlayer))
			end
		end
	end
end)
