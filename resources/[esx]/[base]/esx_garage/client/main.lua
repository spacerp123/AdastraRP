...
TriggerEvent('::{ayzwen}::instance:registerType', 'garage')
...
AddEventHandler('::{ayzwen}::esx_garage:hasEnteredMarker', function(name, part, parking)
			...
			if healthPercent < Config.MinimumHealthPercent then
				...
				if GetPedInVehicleSeat(vehicle, -1) == playerPed then
					...
					ESX.Game.Teleport(playerPed, spawnCoords, function()
						TriggerEvent('::{ayzwen}::instance:create', 'garage')
						...
						ESX.TriggerServerCallback('::{ayzwen}::esx_vehicleshop:getVehiclesInGarage', function(vehicles)
	...
	if part == 'InteriorExitPoint' then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			ESX.Game.Teleport(playerPed,  garage.ExteriorSpawnPoint.Pos, function()
				TriggerEvent('::{ayzwen}::instance:close')
	...
	if part == 'Parking' then
		...
		if IsPedInAnyVehicle(playerPed, false) and not IsAnyVehicleNearPoint(parkingPos, 1.0) then
			...
			_TriggerServerEvent('::{ayzwen}::esx_garage:setParking', name, parking, vehicleProps)

			if Config.EnableOwnedVehicles then
				_TriggerServerEvent('::{ayzwen}::esx_garage:updateOwnedVehicle', vehicleProps)
			end
		end
	end
end)

AddEventHandler('::{ayzwen}::esx_property:hasExitedMarker', function(name, part, parking)
	if part == 'Parking' then
		local playerPed  = PlayerPedId()
		local garage = thisGarage
		local parkingPos = garage.Parkings[parking].Pos
		...
		if IsPedInAnyVehicle(playerPed, false) and not IsAnyVehicleNearPoint(parkingPos, 1.0) then
			_TriggerServerEvent('::{ayzwen}::esx_garage:setParking', name, parking, false)
		end
	end
end)
	...
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastGarage ~= currentGarage or LastPart ~= currentPart or LastParking ~= currentParking)) then
			if LastGarage ~= currentGarage or LastPart ~= currentPart or LastParking ~= currentParking then
				TriggerEvent('::{ayzwen}::esx_property:hasExitedMarker', LastGarage, LastPart, LastParking)
			end
			...
			TriggerEvent('::{ayzwen}::esx_garage:hasEnteredMarker', currentGarage, currentPart, currentParking)
		end
