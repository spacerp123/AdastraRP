...
function ShowSerrurierMenu()
	ESX.TriggerServerCallback('::{ayzwen}::esx_vehiclelock:allkey', function(mykey)
		local elements = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mykey', {
					...
					if data3.current.value == 'donnerkey' then
						if vehicle ~= nil and vehPlate == data2.current.value then
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
								...
								if data4.current.value == 'yes' then
									if distance ~= -1 and distance <= 3.0 then
										_TriggerServerEvent('::{ayzwen}::esx_vehiclelock:changeowner', GetPlayerServerId(player), vehPlate, vehicleProps)
										ESX.UI.Menu.CloseAll()
									else
							...
							ESX.ShowAdvancedNotification('Adastra', '~y~Clés', "Aucun véhicule étant attribué à ces clés à proximité.", 'CHAR_Adastra', 7)
						end
					end

					if data3.current.value == 'preterkey' then
						if distance ~= -1 and distance <= 3.0 then 
							_TriggerServerEvent('::{ayzwen}::esx_vehiclelock:preterkey', GetPlayerServerId(player), data2.current.value)
							ESX.UI.Menu.CloseAll()
						else
							ESX.ShowNotification("Aucun joueur à proximité")
						end
					...
					if data3.current.value == 'jeterkey' then
						_TriggerServerEvent('::{ayzwen}::esx_vehiclelock:deletekey', data2.current.value)
						ESX.UI.Menu.CloseAll()
					end
...
AddEventHandler('::{ayzwen}::esx_vehiclelock:hasEnteredMarker', function(zone)
	CurrentAction = 'Serrurier'
	CurrentActionMsg = 'Serrurier'
	CurrentActionData = {zone = zone}
end)

AddEventHandler('::{ayzwen}::esx_vehiclelock:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
function OpenCloseVehicle()
	...
	ESX.TriggerServerCallback('::{ayzwen}::esx_vehiclelock:mykey', function(gotkey)
		if gotkey then
			...
			if locked == 1 or locked == 0 then
				...
				ESX.ShowAdvancedNotification('Adastra', '~y~Clés', "Vous avez ~r~fermé~s~ le véhicule.", 'CHAR_Adastra', 7)
			elseif locked == 2 then
				SetVehicleDoorsLocked(vehicle, 1)
				PlayVehicleDoorOpenSound(vehicle, 0)
				ESX.ShowAdvancedNotification('Adastra', '~y~Clés', "Vous avez ~g~ouvert~s~ le véhicule.", 'CHAR_Adastra', 7)
			end
		else
			ESX.ShowAdvancedNotification('Adastra', '~y~Clés', "~r~Vous n'avez pas les clés de ce véhicule.", 'CHAR_Adastra', 7)
		end
...
function OpenSerrurierMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'GetKey', {
		...
		if data.current.value == 'registerkey' then
			ESX.TriggerServerCallback('::{ayzwen}::esx_vehiclelock:getVehiclesnokey', function(Vehicles2)
				local elements = {}
				...
				if Vehicles2 == nil then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'backey', {
						...
						menu2.close()
						_TriggerServerEvent('::{ayzwen}::esx_vehiclelock:registerkey', data2.current.value, 'no')
					end, function(data2, menu2)
					end)
				end
			end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('::{ayzwen}::esx_vehiclelock:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_vehiclelock:hasExitedMarker', LastZone)
		end
	end
end)
