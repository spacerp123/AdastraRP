...
function OpenShopMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_main', {
		...
		if data.current.value == 'shop_clothes' then
			...
			TriggerEvent('::{ayzwen}::esx_skin:openRestrictedMenu', function(data2, menu2)
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
					...
					if data3.current.value == 'yes' then
						ESX.TriggerServerCallback('::{ayzwen}::esx_clotheshop:checkMoney', function(hasEnoughMoney)
							if hasEnoughMoney then
								TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
									_TriggerServerEvent('::{ayzwen}::esx_skin:save', skin)
								end)

								_TriggerServerEvent('::{ayzwen}::esx_clotheshop:pay')
								HasPayed = true

								ESX.TriggerServerCallback('::{ayzwen}::esx_clotheshop:checkPropertyDataStore', function(foundStore)
									if foundStore then
										ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing', {
											...
											if data4.current.value == 'yes' then
												ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
													...
													TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
														_TriggerServerEvent('::{ayzwen}::esx_clotheshop:saveOutfit', data5.value, skin)
													end)
								...
								TriggerEvent('::{ayzwen}::esx_skin:getLastSkin', function(skin)
		...
		if data.current.value == 'player_dressing' then
			ESX.TriggerServerCallback('::{ayzwen}::esx_clotheshop:getPlayerDressing', function(dressing)
				local elements = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					...
					TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
						ESX.TriggerServerCallback('::{ayzwen}::esx_clotheshop:getPlayerOutfit', function(clothes)
		...
		if data.current.value == 'suppr_cloth' then
			ESX.TriggerServerCallback('::{ayzwen}::esx_clotheshop:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'supprime_cloth', {
					...
					menu2.close()
					_TriggerServerEvent('::{ayzwen}::esx_clotheshop:deleteOutfit', data2.current.value)
...
AddEventHandler('::{ayzwen}::esx_clotheshop:hasEnteredMarker', function(zone)
	CurrentAction = 'shop_menu'
	CurrentActionMsg = _U('press_menu')
	CurrentActionData = {}
end)

AddEventHandler('::{ayzwen}::esx_clotheshop:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
	CurrentActionMsg = ''
	CurrentActionData = {}
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('::{ayzwen}::esx_clotheshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_clotheshop:hasExitedMarker', LastZone)
		end
	end
end)
