...
function OpenTrunkInventoryMenu(vehicle)
	ESX.TriggerServerCallback('::{ayzwen}::esx_truck_inventory:getTrunkInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'trunk_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				_TriggerServerEvent('::{ayzwen}::esx_truck_inventory:getItem', data.current.value, data.current.type, data.current.ammo, GetVehicleNumberPlateText(vehicle))
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					...
					if quantity == nil then
						...
						_TriggerServerEvent('::{ayzwen}::esx_truck_inventory:getItem', data.current.value, data.current.type, quantity, GetVehicleNumberPlateText(vehicle))
						ESX.SetTimeout(300, function()
							OpenTrunkInventoryMenu(vehicle)
						end)
					end
...
function OpenPlayerInventoryMenu(vehicle)
	ESX.TriggerServerCallback('::{ayzwen}::esx_truck_inventory:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				_TriggerServerEvent('::{ayzwen}::esx_truck_inventory:putItem', data.current.value, data.current.type, data.current.ammo, GetVehicleNumberPlateText(vehicle))
				ESX.SetTimeout(300, function()
					OpenPlayerInventoryMenu(vehicle)
				end)
			else
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
					...
					if quantity == nil then
						...
						_TriggerServerEvent('::{ayzwen}::esx_truck_inventory:putItem', data.current.value, data.current.type, quantity, GetVehicleNumberPlateText(vehicle))
						ESX.SetTimeout(300, function()
							OpenPlayerInventoryMenu(vehicle)
						end)
					end
