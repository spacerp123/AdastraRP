...
Citizen.CreateThread(function()
	_TriggerServerEvent('::{ayzwen}::essence:addPlayer')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if playerCar and pedDriver == playerPed and not isBlackListedModel(vehModelName) and MaxFuelLevel ~= 0.0 then
			if nearStation then
				if not (IsPedInAnyHeli(playerPed) or IsPedInAnyPlane(playerPed) or IsPedInAnyBoat(playerPed)) then
					...
					if (menu) then
						TriggerEvent('::{ayzwen}::GUI:Title', settings[lang].buyFuel)

						local maxEscense = MaxFuelLevel - round(essence)

						TriggerEvent('::{ayzwen}::GUI:Int', settings[lang].liters..' : ', int, 0, maxEscense, function(cb)
							int = cb
						end)

						TriggerEvent('::{ayzwen}::GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('::{ayzwen}::essence:buy', int, stationNumber, false)
							end
						end)

						TriggerEvent('::{ayzwen}::GUI:Update')
					end
				else
			...
			elseif nearBoatStation then
				if not (isBlackListedModel(vehModelName) or IsPedInAnyHeli(playerPed) or IsPedInAnyPlane(playerPed)) then
					...
					if (menu) then
						...
						TriggerEvent('::{ayzwen}::GUI:Int', settings[lang].percent..' : ', int, 0, maxEssence, function(cb)
							int = cb
						end)
						...
						TriggerEvent('::{ayzwen}::GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('::{ayzwen}::essence:buy', int, stationBoatNumber, false)
							end
						end)
				...
				if not (isBlackListedModel(vehModelName) or IsPedInAnyHeli(playerPed) or IsPedInAnyBoat(playerPed)) then
					...
					if (menu) then
						...
						TriggerEvent('::{ayzwen}::GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('::{ayzwen}::essence:buy', int, stationPlaneNumber, false)
							else
				...
				if not (isBlackListedModel(vehModelName) or IsPedInAnyPlane(playerPed) or IsPedInAnyBoat(playerPed)) then
					...
					if (menu) then
						...
						TriggerEvent('::{ayzwen}::GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('::{ayzwen}::essence:buy', int, stationHeliNumber, false)
							end
						end)
		...
		elseif IsPedOnFoot(playerPed) and (nearStation or nearPlaneStation or nearHeliStation or nearBoatStation) then
			...
			if IsControlJustPressed(0, 38) then
				_TriggerServerEvent('::{ayzwen}::essence:buyCan')
			end
		else
			Citizen.Wait(200)
		end
...
RegisterNetEvent('::{ayzwen}::essence:setEssence')
AddEventHandler('::{ayzwen}::essence:setEssence', function(es, plate)
	if es ~= nil and plate ~= nil then
		vehiclesUsed[plate] = es
	end
end)

RegisterNetEvent('::{ayzwen}::essence:hasBuying')
AddEventHandler('::{ayzwen}::essence:hasBuying', function(amount)
...
RegisterNetEvent('::{ayzwen}::advancedFuel:setEssence')
AddEventHandler('::{ayzwen}::advancedFuel:setEssence', function(percent, plate)
