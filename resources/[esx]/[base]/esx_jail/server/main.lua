...
AddEventHandler('::{ayzwen}::esx:playerLoaded', function(playerId, xPlayer)
	MySQL.Async.fetchAll('SELECT jail_time FROM users WHERE identifier = @identifier', {
		...
		if result[1] and result[1].jail_time > 0 then
			TriggerEvent('::{ayzwen}::esx_jail:sendToJail', xPlayer.source, result[1].jail_time, true)
		end
	end)
end)

AddEventHandler('::{ayzwen}::esx:playerDropped', function(playerId, reason)
...
--[[ESX.RegisterCommand('jail', 'admin', function(xPlayer, args, showError)
	TriggerEvent('::{ayzwen}::esx_jail:sendToJail', args.playerId, args.time * 60)
...
RegisterNetEvent('::{ayzwen}::esx_jail:unjail')
AddEventHandler('::{ayzwen}::esx_jail:unjail', function(playerId)
...
RegisterNetEvent('::{ayzwen}::esx_jail:sendToJail')
AddEventHandler('::{ayzwen}::esx_jail:sendToJail', function(playerId, jailTime, quiet)
		...
		if xPlayer then
			if not playersInJail[playerId] then
				MySQL.Async.execute('UPDATE users SET jail_time = @jail_time WHERE identifier = @identifier', {
					...
					xPlayer.triggerEvent('::{ayzwen}::esx_policejob:unrestrain')
					xPlayer.triggerEvent('::{ayzwen}::esx_jail:jailPlayer', jailTime)
	...
	if xPlayer then
		if playersInJail[playerId] then
			MySQL.Async.execute('UPDATE users SET jail_time = 0 WHERE identifier = @identifier', {
				...
				playersInJail[playerId] = nil
				xPlayer.triggerEvent('::{ayzwen}::esx_jail:unjailPlayer')
			end)
		end
	end
end
