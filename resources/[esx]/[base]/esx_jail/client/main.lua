...
RegisterNetEvent('::{ayzwen}::esx_jail:jailPlayer')
AddEventHandler('::{ayzwen}::esx_jail:jailPlayer', function(_jailTime)
	jailTime = _jailTime
	...
	TriggerEvent('::{ayzwen}::skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			TriggerEvent('::{ayzwen}::skinchanger:loadClothes', skin, Config.Uniforms.prison_wear.male)
		else
			TriggerEvent('::{ayzwen}::skinchanger:loadClothes', skin, Config.Uniforms.prison_wear.female)
		end
	end)
	...
	while not unjail do
		...
		if #(GetEntityCoords(playerPed) - Config.JailLocation) > 10 then
			ESX.Game.Teleport(playerPed, Config.JailLocation)
			TriggerEvent('::{ayzwen}::chat:addMessage', {args = {_U('judge'), _U('escape_attempt')}, color = {147, 196, 109}})
		end
	end
...
RegisterNetEvent('::{ayzwen}::esx_jail:unjailPlayer')
AddEventHandler('::{ayzwen}::esx_jail:unjailPlayer', function()
