...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('::{ayzwen}::esx_shops:requestDBItems', function(dbItems)
		shopItems = dbItems
	end)
end)

AddEventHandler('::{ayzwen}::esx_shops:hasEnteredMarker', function()
	CurrentAction = 'shop_menu'
	CurrentActionMsg = _U('press_menu')
end)

AddEventHandler('::{ayzwen}::esx_shops:hasExitedMarker', function()
	CurrentAction = nil
	RageUI.Visible(RMenu.Get('rageui', 'shop'), false)
end)

RegisterNetEvent('::{ayzwen}::esx_shops:removePickup')
AddEventHandler('::{ayzwen}::esx_shops:removePickup', function(bank)
	RemoveBlip(robBlip)
	for i = 1, #objects do
...
RegisterNetEvent('::{ayzwen}::esx_shops:robberyOver')
AddEventHandler('::{ayzwen}::esx_shops:robberyOver', function()
	Citizen.Wait(10000)
	robbing = false
end)

RegisterNetEvent('::{ayzwen}::esx_shops:talk')
AddEventHandler('::{ayzwen}::esx_shops:talk', function(store, text, time)
	robbing = false
...
RegisterNetEvent('::{ayzwen}::esx_shops:rob')
						...
						while true do
							...
							if DoesEntityExist(object) then
								if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(object)) <= 1.5 then
									...
									_TriggerServerEvent('::{ayzwen}::esx_shops:pickUp', store)
									break
								end
							else
								break
...
RegisterNetEvent('::{ayzwen}::esx_shops:resetStore')
AddEventHandler('::{ayzwen}::esx_shops:resetStore', function(store)
...
RegisterNetEvent('::{ayzwen}::esx_shops:msgPolice')
AddEventHandler('::{ayzwen}::esx_shops:msgPolice', function(store, coords)
	...
	while true do
		...
		for i = 1, #Config.Zones, 1 do
			if Config.Zones[i].ped.handle and IsPedDeadOrDying(Config.Zones[i].ped.handle) then
				_TriggerServerEvent('::{ayzwen}::esx_shops:pedDead', i)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('::{ayzwen}::esx_shops:hasEnteredMarker')
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_shops:hasExitedMarker')
		end
	end
end)
			...
			for i = 1, #Config.Zones do
				if Config.Zones[i].ped.handle then
					...
					if HasEntityClearLosToEntityInFront(plyPed, shopPed, 19) and not IsPedDeadOrDying(shopPed) and #(plyCoords - GetEntityCoords(shopPed)) <= 5.0 then
						if not robbing then
							ESX.TriggerServerCallback('::{ayzwen}::esx_shops:canRob', function(enoughCops, canRob)
								if enoughCops and canRob then
									...
									_TriggerServerEvent('::{ayzwen}::esx_shops:policeNotif', i, GetEntityCoords(shopPed))
									...
									if #(GetEntityCoords(plyPed) - GetEntityCoords(shopPed)) <= 7.5 then
										if not IsPedDeadOrDying(shopPed) then
											_TriggerServerEvent('::{ayzwen}::esx_shops:rob', i)
								...
								elseif enoughCops and not canRob then
									TriggerEvent('::{ayzwen}::esx_shops:talk', i, 'Je me suis déjà fais volé la caisse est vide', 5)
									Citizen.Wait(2500)
...
RageUI.CreateWhile(1.0, nil, nil, function()
	RageUI.IsVisible(RMenu.Get('rageui', 'shop'), true, false, true, function()
		for i = 1, #shopItems, 1 do
			RageUI.Button(shopItems[i].label, 'Bienvenue à vous ! Passez votre commande.', {RightLabel = ('%s$'):format(shopItems[i].price)}, true, function(Hovered, Active, Selected)
				if Selected then
					_TriggerServerEvent('::{ayzwen}::esx_shops:buyItem', shopItems[i].value, 1, CurrentZone)
				end
			end)
		end
	end, function()
