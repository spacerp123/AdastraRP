...
RegisterNetEvent('::{ayzwen}::esx_weashop:loadLicenses')
AddEventHandler('::{ayzwen}::esx_weashop:loadLicenses', function(licenses)
...
function OpenBuyLicenseMenu(categorySelected)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_buy_license', {
		...
		if data.current.value == 'yes' then
			_TriggerServerEvent('::{ayzwen}::esx_weashop:buyLicense', licenseType, categorySelected)
		end
...
function OpenShopMenu(categorySelected)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_buy_weapons', {
		...
		_TriggerServerEvent('::{ayzwen}::esx_weashop:buyItem', data.current.value, categorySelected)
	end, function(data, menu)
	end)
end

AddEventHandler('::{ayzwen}::esx_weashop:hasEnteredMarker', function()
	CurrentAction = 'shop_menu'
	CurrentActionMsg = _U('shop_menu')
	CurrentActionData = {}
end)

AddEventHandler('::{ayzwen}::esx_weashop:hasExitedMarker', function()
	CurrentAction = nil
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('::{ayzwen}::esx_weashop:hasEnteredMarker')
		end
	
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::esx_weashop:hasExitedMarker')
		end
	end
end)
...
RegisterNetEvent('::{ayzwen}::esx_weashop:useClip')
AddEventHandler('::{ayzwen}::esx_weashop:useClip', function()
	...
	if IsPedArmed(playerPed, 4) then
		...
		if hash then
			_TriggerServerEvent('::{ayzwen}::esx_weashop:removeClip')
