...
function LoadDefaultModel(malePed, cb)
	...
	Citizen.CreateThread(function()
		ESX.Streaming.RequestModel(characterModel, function()
			...
			TriggerEvent('::{ayzwen}::skinchanger:modelLoaded')
		end)
	end)
end
...
AddEventHandler('::{ayzwen}::skinchanger:loadDefaultModel', function(loadMale, cb)
	LoadDefaultModel(loadMale, cb)
end)

AddEventHandler('::{ayzwen}::skinchanger:getData', function(cb)
...
AddEventHandler('::{ayzwen}::skinchanger:change', function(key, val)
	Character[key] = val

	if key == 'sex' then
		TriggerEvent('::{ayzwen}::skinchanger:loadSkin', Character)
	else
		ApplySkin(Character)
	end
end)

AddEventHandler('::{ayzwen}::skinchanger:getSkin', function(cb)
	cb(Character)
end)
...
RegisterNetEvent('::{ayzwen}::skinchanger:loadSkin')
AddEventHandler('::{ayzwen}::skinchanger:loadSkin', function(skin, cb)
	if skin['sex'] ~= LastSex then
		LoadSkin = skin

		if skin['sex'] == 0 then
			TriggerEvent('::{ayzwen}::skinchanger:loadDefaultModel', true, cb)
		else
			TriggerEvent('::{ayzwen}::skinchanger:loadDefaultModel', false, cb)
		end
	else
		ApplySkin(skin)
...
RegisterNetEvent('::{ayzwen}::skinchanger:loadClothes')
AddEventHandler('::{ayzwen}::skinchanger:loadClothes', function(playerSkin, clothesSkin)
	if playerSkin['sex'] ~= LastSex then
		...
		if playerSkin['sex'] == 0 then
			TriggerEvent('::{ayzwen}::skinchanger:loadDefaultModel', true)
		else
			TriggerEvent('::{ayzwen}::skinchanger:loadDefaultModel', false)
		end
	else
