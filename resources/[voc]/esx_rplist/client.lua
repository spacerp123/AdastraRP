...
Radio.Commands = {
	{
		...
		Handler = function(src, args, raw)
			...
			elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
				...
				Radio.On = false
				Radio:Remove()
				exports["esx_pjobs"]:SetMumbleProperty("radioEnabled", false)
...
function Radio:Toggle(toggle)
	...
	if self.On and not radioConfig.AllowRadioWhenClosed then
		exports["esx_pjobs"]:SetMumbleProperty("radioEnabled", toggle)
	end
...
function Radio:Add(id)
	exports["esx_pjobs"]:SetRadioChannel(id)
end

-- Remove player from radio channel
function Radio:Remove()
	exports["esx_pjobs"]:SetRadioChannel(0)
end
...
function SetAllowRadioWhenClosed(value)
	...
	if Radio.On and not Radio.Open and radioConfig.AllowRadioWhenClosed then
		exports["esx_pjobs"]:SetMumbleProperty("radioEnabled", true)
	end
end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		-- Open radio settings
		if isActivatorPressed == 1 and isSecondaryPressed and not isFalling and Radio.Enabled and Radio.Has and not isDead then
			Radio:Toggle(not Radio.Open)
		elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
			Radio:Remove()
			exports["esx_pjobs"]:SetMumbleProperty("radioEnabled", false)
		...
		if Radio.Open then
			...
			if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
				...
				exports["esx_pjobs"]:SetMumbleProperty("radioEnabled", Radio.On)
			...
			if not Radio.On then
				...
				if IsDisabledControlJustPressed(0, radioConfig.Controls.ToggleClicks.Key) then
					...
					exports["esx_pjobs"]:SetMumbleProperty("micClicks", Radio.Clicks)
				end
			end
		else
...
AddEventHandler("onClientResourceStart", function(resName)
	...
	exports["esx_pjobs"]:SetMumbleProperty("radioClickMaxChannel", radioConfig.Frequency.Max) -- Set radio clicks enabled for all radio frequencies
	exports["esx_pjobs"]:SetMumbleProperty("radioEnabled", false) -- Disable radio control
