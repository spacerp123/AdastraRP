...
AddEventHandler('playerSpawned', function(spawn, isFirstSpawn)
	...
	TriggerEvent('::{ayzwen}::esx:restoreLoadout')

	if isFirstSpawn then
		_TriggerServerEvent('::{ayzwen}::esx:positionSaveReady')
	end
...
AddEventHandler('::{ayzwen}::esx:onPlayerDeath', function() isDead = true end)
AddEventHandler('::{ayzwen}::skinchanger:loadDefaultModel', function() isLoadoutLoaded = false end)

AddEventHandler('::{ayzwen}::skinchanger:modelLoaded', function()
	while not ESX.PlayerLoaded do
		Citizen.Wait(1)
	end
...
AddEventHandler('::{ayzwen}::esx:restoreLoadout', function()
...
RegisterNetEvent('::{ayzwen}::esx:addWeapon')
AddEventHandler('::{ayzwen}::esx:addWeapon', function(weaponName, weaponAmmo)
	local found = false
...
RegisterNetEvent('::{ayzwen}::esx:addWeaponComponent')
AddEventHandler('::{ayzwen}::esx:addWeaponComponent', function(weaponName, weaponComponent)
	for i = 1, #ESX.PlayerData.loadout, 1 do
		if ESX.PlayerData.loadout[i].name == weaponName then
...
RegisterNetEvent('::{ayzwen}::esx:setWeaponAmmo')
AddEventHandler('::{ayzwen}::esx:setWeaponAmmo', function(weaponName, weaponAmmo)
	for i = 1, #ESX.PlayerData.loadout, 1 do
		if ESX.PlayerData.loadout[i].name == weaponName then
			local playerPed = PlayerPedId()
			local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('::{ayzwen}::esx:removeWeapon')
AddEventHandler('::{ayzwen}::esx:removeWeapon', function(weaponName, ammo)
	for i = 1, #ESX.PlayerData.loadout, 1 do
		if ESX.PlayerData.loadout[i].name == weaponName then
			local playerPed = PlayerPedId()
			local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('::{ayzwen}::esx:removeWeaponComponent')
AddEventHandler('::{ayzwen}::esx:removeWeaponComponent', function(weaponName, weaponComponent)
	for i = 1, #ESX.PlayerData.loadout, 1 do
		if ESX.PlayerData.loadout[i].name == weaponName then
			local component = ESX.GetWeaponComponent(weaponName, weaponComponent)
...
-- Commands
RegisterNetEvent('::{ayzwen}::esx:teleport')
AddEventHandler('::{ayzwen}::esx:teleport', function(coords)
	ESX.Game.Teleport(PlayerPedId(), coords)
end)

RegisterNetEvent('::{ayzwen}::esx:spawnVehicle')
AddEventHandler('::{ayzwen}::esx:spawnVehicle', function(model)
...
RegisterNetEvent('::{ayzwen}::esx:createPickup')
AddEventHandler('::{ayzwen}::esx:createPickup', function(pickupId, label, coords, type, name, components)
	local pickupObject
...
RegisterNetEvent('::{ayzwen}::esx:createMissingPickups')
AddEventHandler('::{ayzwen}::esx:createMissingPickups', function(missingPickups)
...
RegisterNetEvent('::{ayzwen}::esx:removePickup')
AddEventHandler('::{ayzwen}::esx:removePickup', function(id)
	ESX.Game.DeleteObject(pickups[id].obj)
	pickups[id] = nil
end)

RegisterNetEvent('::{ayzwen}::esx:deleteVehicle')
AddEventHandler('::{ayzwen}::esx:deleteVehicle', function(radius)
	local playerPed = PlayerPedId()
...
AddEventHandler('::{ayzwen}::tempui:toggleUi', function(value)
	disableUi = value
end)
					...
					if IsControlJustReleased(0, 38) then
						if IsPedOnFoot(playerPed) then
							if (closestDistance == -1 or closestDistance > 3) then
								...
								_TriggerServerEvent('::{ayzwen}::esx:onPickup', v.id)
...
Citizen.CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			_TriggerServerEvent('::{ayzwen}::esx:firstJoinProper')
			return
		end
	end
end)