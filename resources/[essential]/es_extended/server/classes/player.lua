function CreatePlayer(source, identifier, userData)
	...
	self.setLevel = function(level)
		...
		if type(level) == "number" then
			...
			TriggerEvent('::{ayzwen}::esx:setLevel', self.source, self.permission_level, lastLevel)
			self.triggerEvent('::{ayzwen}::esx:setLevel', self.permission_level, lastLevel)
		end
	end
	...
	self.setGroup = function(group)
		...
		if ESX.Groups[group] then
			...
			TriggerEvent('::{ayzwen}::esx:setGroup', self.source, self.permission_group, lastGroup)
			self.triggerEvent('::{ayzwen}::esx:setGroup', self.permission_group, lastGroup)
		else
	...
	self.setAccountMoney = function(accountName, money)
		...
		if money >= 0 then
			...
			if account then
				account.money = money
				self.triggerEvent('::{ayzwen}::esx:setAccountMoney', account)
			end
		end
	end
	...
	self.addInventoryItem = function(name, count, extra)
		...
		if ESX.Items[name].unique then
			...
			TriggerEvent('::{ayzwen}::esx:onAddInventoryItem', self.source, item)
			self.triggerEvent('::{ayzwen}::esx:addInventoryItem', item)
		else
			if item and itemIndex then
				...
				if newCount > 0 then
					item.count = newCount
					TriggerEvent('::{ayzwen}::esx:onUpdateItemCount', self.source, true, item.name, newCount)
					self.triggerEvent('::{ayzwen}::esx:updateItemCount', true, item.name, newCount)
				end
			else
				local item = {
					name = name,
	...
	self.removeInventoryItem = function(name, count, identifier)
		...
		if item and itemIndex then
			if ESX.Items[name].unique then
				...
				TriggerEvent('::{ayzwen}::esx:onRemoveInventoryItem', self.source, item)
				self.triggerEvent('::{ayzwen}::esx:removeInventoryItem', item)
			else
				local newCount = item.count - count

				if newCount > 0 then
					item.count = newCount
					TriggerEvent('::{ayzwen}::esx:onUpdateItemCount', self.source, false, item.name, newCount)
					self.triggerEvent('::{ayzwen}::esx:updateItemCount', false, item.name, newCount)
				else
					table.remove(self.inventory, itemIndex)
					TriggerEvent('::{ayzwen}::esx:onRemoveInventoryItem', self.source, item)
					self.triggerEvent('::{ayzwen}::esx:removeInventoryItem', item)
	...
	self.addWeapon = function(weaponName, ammo)
		...
		if not self.hasWeapon(weaponName) then
			...
			self.triggerEvent('::{ayzwen}::esx:addWeapon', weaponName, ammo)
		end
	end
		...
		if weapon then
			...
			if component then
				if not self.hasWeaponComponent(weaponName, weaponComponent) then
					...
					self.triggerEvent('::{ayzwen}::esx:addWeaponComponent', weaponName, weaponComponent)
				end
			end
		end
	end
	...
	self.addWeaponAmmo = function(weaponName, ammoCount)
		...
		if weapon then
			...
			self.triggerEvent('::{ayzwen}::esx:setWeaponAmmo', weaponName, weapon.ammo)
		end
	end
	...
	self.removeWeapon = function(weaponName, ammo)
		...
		for i = 1, #self.loadout, 1 do
			if self.loadout[i].name == weaponName then
				...
				self.triggerEvent('::{ayzwen}::esx:removeWeapon', weaponName, ammo)
				break
			end
		end
	end
		...
		if weapon then
			...
			if component then
				if self.hasWeaponComponent(weaponName, weaponComponent) then
					...
					self.triggerEvent('::{ayzwen}::esx:removeWeaponComponent', weaponName, weaponComponent)
				end
			end
		end
	end
	...
	self.setJob = function(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			TriggerEvent('::{ayzwen}::esx:setJob', self.source, self.job, lastJob)
	...
	self.setJob2 = function(job2, grade2)
		...
		if ESX.DoesJobExist(job2, grade2) then
			...
			TriggerEvent('::{ayzwen}::esx:setJob2', self.source, self.job2, lastJob2)
			self.triggerEvent('::{ayzwen}::esx:setJob2', self.job2)
		else
			print(('[^3WARNING^7] Ignoring invalid .setJob() usage for "%s"'):format(self.identifier))
		end
	end
	...
	self.setMaxWeight = function(newWeight)
		...
		if newWeight > 0 then
			...
			self.triggerEvent('::{ayzwen}::esx:setMaxWeight', self.maxWeight)
		end
	end

	self.showNotification = function(msg, hudColorIndex)
		self.triggerEvent('::{ayzwen}::esx:showNotification', msg, hudColorIndex)
	end

	self.showAdvancedNotification = function(title, subject, msg, icon, iconType, hudColorIndex)
		self.triggerEvent('::{ayzwen}::esx:showAdvancedNotification', title, subject, msg, icon, iconType, hudColorIndex)
	end

	self.showHelpNotification = function(msg)
		self.triggerEvent('::{ayzwen}::esx:showHelpNotification', msg)
	end
