function LoadUser(source, identifier)
	...
	Async.parallel(tasks, function(results)
		...
		TriggerEvent('::{ayzwen}::esx:playerLoaded', source, xPlayer)

		xPlayer.triggerEvent('::{ayzwen}::esx:playerLoaded', {
		...
		xPlayer.triggerEvent('::{ayzwen}::esx:createMissingPickups', ESX.Pickups)
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		TriggerEvent('::{ayzwen}::esx:playerDropped', _source, xPlayer, reason)
...
RegisterServerEvent('::{ayzwen}::esx:firstJoinProper')
AddEventHandler('::{ayzwen}::esx:firstJoinProper', function()
	local _source = source
...
RegisterServerEvent('::{ayzwen}::esx:giveInventoryItem')
AddEventHandler('::{ayzwen}::esx:giveInventoryItem', function(target, type, itemName, itemCount)
	local _source = source
...
RegisterServerEvent('::{ayzwen}::esx:dropInventoryItem')
AddEventHandler('::{ayzwen}::esx:dropInventoryItem', function(type, itemName, itemCount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('::{ayzwen}::esx:useItem')
AddEventHandler('::{ayzwen}::esx:useItem', function(itemName)
...
RegisterServerEvent('::{ayzwen}::esx:onPickup')
AddEventHandler('::{ayzwen}::esx:onPickup', function(id)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if pickup then
		...
		if success then
			ESX.Pickups[id] = nil
			TriggerClientEvent('::{ayzwen}::esx:removePickup', -1, id)
		end
	end
end)

RegisterServerEvent('::{ayzwen}::esx:positionSaveReady')
AddEventHandler('::{ayzwen}::esx:positionSaveReady', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.positionSaveReady = true
end)

ESX.RegisterServerCallback('::{ayzwen}::esx:getPlayerData', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('::{ayzwen}::esx:getOtherPlayerData', function(source, cb, target)
...
ESX.RegisterServerCallback('::{ayzwen}::esx:getActivePlayers', function(source, cb)
	local players = {}
