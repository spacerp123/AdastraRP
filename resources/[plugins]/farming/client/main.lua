...
AddEventHandler('::{ayzwen}::farming:hasEnteredMarker', function(zone, type)
...
AddEventHandler('::{ayzwen}::farming:hasExitedMarker', function(zone, type)
	CurrentAction = {zone = nil, type = nil}
	_TriggerServerEvent('::{ayzwen}::farming:stopAction', zone, type)
end)

RegisterNetEvent('::{ayzwen}::farming:changeMarker')
AddEventHandler('::{ayzwen}::farming:changeMarker', function(zone, type)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('::{ayzwen}::farming:hasEnteredMarker', currentZone, currentType)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('::{ayzwen}::farming:hasExitedMarker', lastZone, lastType)
		end
	end
end)
	...
	while true do
		...
		if CurrentAction.zone ~= nil and CurrentAction.type ~= nil then
			...
			if IsControlJustReleased(0, 38) and IsPedOnFoot(PlayerPedId()) then
				_TriggerServerEvent('::{ayzwen}::farming:startAction', CurrentAction.zone, CurrentAction.type)
