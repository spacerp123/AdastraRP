...
Citizen.CreateThread(function()
	...
	for i = 1, #GangsData, 1 do
		TriggerEvent('::{ayzwen}::esx_society:registerSociety', GangsData[i].Name, GangsData[i].Label, 'society_' .. GangsData[i].Name, 'society_' .. GangsData[i].Name, 'society_' .. GangsData[i].Name, {type = 'public'})
	end
end)
...
RegisterServerEvent('::{ayzwen}::GangsBuilder:addGang')
AddEventHandler('::{ayzwen}::GangsBuilder:addGang', function(data)
...
RegisterServerEvent('::{ayzwen}::GangsBuilder:requestSync')
AddEventHandler('::{ayzwen}::GangsBuilder:requestSync', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	local plyGang = GetGang(xPlayer.job2)
	TriggerClientEvent('::{ayzwen}::GangsBuilder:SyncGang', xPlayer.source, plyGang)
end)

AddEventHandler('::{ayzwen}::esx:playerLoaded', function(source, xPlayer)
	local plyGang = GetGang(xPlayer.job2)
	TriggerClientEvent('::{ayzwen}::GangsBuilder:SyncGang', source, plyGang)
end)

AddEventHandler('::{ayzwen}::esx:setJob2', function(source, job2)
	local plyGang = GetGang(job2)
	TriggerClientEvent('::{ayzwen}::GangsBuilder:SyncGang', source, plyGang)
end)

ESX.AddGroupCommand('gangsbuilder', 'superadmin', function(source)
	TriggerClientEvent('::{ayzwen}::GangsBuilder:OpenMenu', source)
end, {help = ''})