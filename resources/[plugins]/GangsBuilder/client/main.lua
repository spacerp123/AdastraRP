...
Citizen.CreateThread(function()
	...
	_TriggerServerEvent('::{ayzwen}::GangsBuilder:requestSync')
end)
...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'gang_wear' then
			ESX.TriggerServerCallback('::{ayzwen}::esx_skin:getPlayerSkin', function(skin, jobSkin, job2Skin)
				if skin.sex == 0 then
					TriggerEvent('::{ayzwen}::skinchanger:loadClothes', skin, job2Skin.skin_male)
				else
					TriggerEvent('::{ayzwen}::skinchanger:loadClothes', skin, job2Skin.skin_female)
				end
			end)
		end
...
function OpenArmoryMenu()
	...
	table.insert(elements, {label = _U('zizi'), value = 'zizi'})
...
function OpenVehicleSpawnerMenu()
	...
	ESX.TriggerServerCallback('::{ayzwen}::esx_society:getVehiclesInGarage', function(vehicles)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
			...
			_TriggerServerEvent('::{ayzwen}::esx_society:removeVehicleFromGarage', ActualGang.Name, vehicleProps)
		end, function(data, menu)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if distance ~= -1 and distance <= 3.0 then
					...
					if data2.current.value == 'put_in_vehicle' then
						_TriggerServerEvent('::{ayzwen}::GangsBuilder:putInVehicle', GetPlayerServerId(player))
					end

					if data2.current.value == 'out_the_vehicle' then
						_TriggerServerEvent('::{ayzwen}::GangsBuilder:OutVehicle', GetPlayerServerId(player))
					end
				else
...
function OpenBodySearchMenu(player)
	ESX.TriggerServerCallback('::{ayzwen}::GangsBuilder:getOtherPlayerData', function(data)
		if data.foundPlayer then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
				...
				if data2.current.value ~= nil then
					menu2.close()
					_TriggerServerEvent('::{ayzwen}::GangsBuilder:confiscatePlayerItem', player, data2.current.itemType, data2.current.value, data2.current.amount)
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('::{ayzwen}::GangsBuilder:getVehicleInfos', function(infos)
		local elements = {}
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('::{ayzwen}::GangsBuilder:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('::{ayzwen}::GangsBuilder:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('::{ayzwen}::GangsBuilder:addArmoryWeapon', function()
			OpenPutWeaponMenu()
		end, data.current.value, data.current.ammo)
	end, function(data, menu)
	end)
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::GangsBuilder:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::GangsBuilder:getStockItem', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{ayzwen}::GangsBuilder:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{ayzwen}::GangsBuilder:putStockItems', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
AddEventHandler('::{ayzwen}::GangsBuilder:hasEnteredMarker', function(part)
...
AddEventHandler('::{ayzwen}::GangsBuilder:hasExitedMarker', function(part)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

RegisterNetEvent('::{ayzwen}::GangsBuilder:putInVehicle')
AddEventHandler('::{ayzwen}::GangsBuilder:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
...
RegisterNetEvent('::{ayzwen}::GangsBuilder:OutVehicle')
AddEventHandler('::{ayzwen}::GangsBuilder:OutVehicle', function()
		...
		if PlayerData.job2 ~= nil and ActualGang then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastPart ~= currentPart)) then
				if (LastPart ~= nil) and (LastPart ~= currentPart) then
					TriggerEvent('::{ayzwen}::GangsBuilder:hasExitedMarker', LastPart)
					hasExited = true
				end
				...
				TriggerEvent('::{ayzwen}::GangsBuilder:hasEnteredMarker', currentPart)
			end
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0, 38) and PlayerData.job2 ~= nil and ActualGang and (GetGameTimer() - GUI.Time) > 150 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					_TriggerServerEvent('::{ayzwen}::esx_society:putVehicleInGarage', ActualGang.Name, vehicleProps)
				...
				if CurrentAction == 'menu_boss_actions' then
					...
					TriggerEvent('::{ayzwen}::esx_society:openBossMenu2', ActualGang.Name, function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, {wash = false})
