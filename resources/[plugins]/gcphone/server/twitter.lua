        ...
        MySQL.Async.execute("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
            ...
            MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
                ...
                if tweet then
                    ...
                    TriggerClientEvent('::{ayzwen}::gcPhone:twitter_newTweets', -1, tweet)
                    TriggerEvent('::{ayzwen}::gcPhone:twitter_newTweets', tweet)
                end
            end)
        end)
    end)
				...
				if row[1] == nil then
					MySQL.Async.execute('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
						...
						MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
							...
							TriggerClientEvent('::{ayzwen}::gcPhone:twitter_updateTweetLikes', -1, tweet.id, tweet.likes + 1)
							TriggerClientEvent('::{ayzwen}::gcPhone:twitter_setTweetLikes', sourcePlayer, tweet.id, true)
							TriggerEvent('::{ayzwen}::gcPhone:twitter_updateTweetLikes', tweet.id, tweet.likes + 1)
						end)
					end)
				else
					MySQL.Async.execute('DELETE FROM twitter_likes WHERE id = @id', {
						...
						MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
							...
							TriggerClientEvent('::{ayzwen}::gcPhone:twitter_updateTweetLikes', -1, tweet.id, tweet.likes - 1)
							TriggerClientEvent('::{ayzwen}::gcPhone:twitter_setTweetLikes', sourcePlayer, tweet.id, false)
							TriggerEvent('::{ayzwen}::gcPhone:twitter_updateTweetLikes', tweet.id, tweet.likes - 1)
						end)
					end)
				end
...
function TwitterShowError(sourcePlayer, title, message)
	TriggerClientEvent('::{ayzwen}::gcPhone:twitter_showError', sourcePlayer, message)
end

function TwitterShowSuccess(sourcePlayer, title, message)
	TriggerClientEvent('::{ayzwen}::gcPhone:twitter_showSuccess', sourcePlayer, title, message)
end

RegisterServerEvent('::{ayzwen}::gcPhone:twitter_login')
AddEventHandler('::{ayzwen}::gcPhone:twitter_login', function(username, password)
	...
	getUser(username, password, function(user)
		if user == nil then
			...
			TriggerClientEvent('::{ayzwen}::gcPhone:twitter_setAccount', sourcePlayer, username, password, user.authorIcon)
		end
	end)
end)

RegisterServerEvent('::{ayzwen}::gcPhone:twitter_changePassword')
AddEventHandler('::{ayzwen}::gcPhone:twitter_changePassword', function(username, password, newPassword)
	local sourcePlayer = tonumber(source)
	getUser(username, password, function(user)
		if user == nil then
			...
			MySQL.Async.execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
				...
				if (result == 1) then
					TriggerClientEvent('::{ayzwen}::gcPhone:twitter_setAccount', sourcePlayer, username, newPassword, user.authorIcon)
...
RegisterServerEvent('::{ayzwen}::gcPhone:twitter_createAccount')
AddEventHandler('::{ayzwen}::gcPhone:twitter_createAccount', function(username, password, avatarUrl)
	local sourcePlayer = tonumber(source)

	TwitterCreateAccount(username, password, avatarUrl, function(id)
		if id ~= 0 then
			TriggerClientEvent('::{ayzwen}::gcPhone:twitter_setAccount', sourcePlayer, username, password, avatarUrl)
...
RegisterServerEvent('::{ayzwen}::gcPhone:twitter_getTweets')
AddEventHandler('::{ayzwen}::gcPhone:twitter_getTweets', function(username, password)
	local sourcePlayer = tonumber(source)
	if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
		getUser(username, password, function(user)
			local accountId = user and user.id
			TwitterGetTweets(accountId, function(tweets)
				TriggerClientEvent('::{ayzwen}::gcPhone:twitter_getTweets', sourcePlayer, tweets)
			end)
		end)
	else
...
RegisterServerEvent('::{ayzwen}::gcPhone:twitter_getFavoriteTweets')
AddEventHandler('::{ayzwen}::gcPhone:twitter_getFavoriteTweets', function(username, password)
	local sourcePlayer = tonumber(source)
	if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
		getUser(username, password, function(user)
			local accountId = user and user.id
			TwitterGetFavotireTweets(accountId, function(tweets)
				TriggerClientEvent('::{ayzwen}::gcPhone:twitter_getFavoriteTweets', sourcePlayer, tweets)
			end)
		end)
	else
...
RegisterServerEvent('::{ayzwen}::gcPhone:twitter_postTweets')
AddEventHandler('::{ayzwen}::gcPhone:twitter_postTweets', function(username, password, message)
	local sourcePlayer = tonumber(source)
...
RegisterServerEvent('::{ayzwen}::gcPhone:twitter_toogleLikeTweet')
AddEventHandler('::{ayzwen}::gcPhone:twitter_toogleLikeTweet', function(username, password, tweetId)
	local sourcePlayer = tonumber(source)
	TwitterToogleLike(username, password, tweetId, sourcePlayer)
end)

RegisterServerEvent('::{ayzwen}::gcPhone:twitter_setAvatarUrl')
AddEventHandler('::{ayzwen}::gcPhone:twitter_setAvatarUrl', function(username, password, avatarUrl)
	local sourcePlayer = tonumber(source)
...
AddEventHandler('::{ayzwen}::gcPhone:twitter_newTweets', function(tweet)
