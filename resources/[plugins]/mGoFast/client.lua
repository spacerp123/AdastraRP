...
CreateThread(function() while ESX == nil do TriggerEvent("::{ayzwen}::esx:getSharedObject", function(obj) ESX = obj end) Wait(10) end end)
...
RegisterNetEvent("Azw:AdvancedNotif") 
AddEventHandler("Azw:AdvancedNotif", function(title, subject, msg, icon, iconType) 
...
function TypesTrajets(typetrajet, nameveh, coords, heading)
    if typetrajet == 1 then
        print("Functions >> Drogues Loaded")
        ESX.Game.SpawnVehicle(nameveh, coords, heading, function(vehicle1) 
            ...
            SetTimeout(1 * 60000, function()
                TriggerServEvent("Ayzwen:AlertPolice", "Livraison", "Illégal", "Un citoyen a apercu un véhicule avec une cargaison illégal dans le nord!", "CHAR_CALL911", 1)
            end)
        end)
    elseif typetrajet == 2 then
        print("Functions >> Argents Sale Loaded")
        ...
        for _, var in pairs(GoFastConfig.RacheteurDrogues) do
            if MissIsActived then
                if #(pCoords2 - var.PositionRacheteur) < 2.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        TriggerServEvent("Ayzwen:PaymentGoFastDrogues", GoFastConfig.PriceRandom.PriceDrogues)
                        activerfps = false
        ...
        for _, var in pairs(GoFastConfig.RacheteurArgentSale) do
            if MissIsActived then
                if #(pCoords2 - var.PositionRacheteur) < 2.0 then
                    ...
                    Visual.Subtitle("Appuyez sur ~o~E~s~ pour completer la mission", 1)
                    if IsControlJustReleased(0, 38) then
                        TriggerServEvent("Ayzwen:PaymentGoFastArgent", GoFastConfig.PriceRandom.PriceArgentSale)
                        activerfps = false
                        Visual.Subtitle("Bienvue mon gars ! revient plus tard", 6500)
                        Wait(10 * 60000)
                        activerfps = true
