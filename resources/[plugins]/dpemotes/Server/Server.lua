...
RegisterServerEvent("::{ayzwen}::ServerEmoteRequest")
AddEventHandler("::{ayzwen}::ServerEmoteRequest", function(target, emotename, etype)
	TriggerClientEvent("::{ayzwen}::ClientEmoteRequestReceive", target, emotename, etype)
end)

RegisterServerEvent("::{ayzwen}::ServerValidEmote")
AddEventHandler("::{ayzwen}::ServerValidEmote", function(target, requestedemote, otheremote)
	TriggerClientEvent("::{ayzwen}::SyncPlayEmote", source, otheremote, source)
	TriggerClientEvent("::{ayzwen}::SyncPlayEmoteSource", target, requestedemote)
end)
...
MySQL.ready(function()
	RegisterServerEvent("::{ayzwen}::dp:ServerKeybindExist")
	AddEventHandler('::{ayzwen}::dp:ServerKeybindExist', function()
		...
		MySQL.Async.fetchAll('SELECT * FROM dpkeybinds WHERE `id`=@id;', {id = srcid}, function(dpkeybinds)
			if dpkeybinds[1] then
				TriggerClientEvent("::{ayzwen}::dp:ClientKeybindExist", src, true)
			else
				TriggerClientEvent("::{ayzwen}::dp:ClientKeybindExist", src, false)
			end
		end)
	end)
	...
	RegisterServerEvent("::{ayzwen}::dp:ServerKeybindCreate")
	AddEventHandler("::{ayzwen}::dp:ServerKeybindCreate", function()
		local src = source local srcid = GetPlayerIdentifier(source)
		...
		MySQL.Async.execute('INSERT INTO dpkeybinds (`id`, `keybind1`, `emote1`, `keybind2`, `emote2`, `keybind3`, `emote3`, `keybind4`, `emote4`, `keybind5`, `emote5`, `keybind6`, `emote6`) VALUES (@id, @keybind1, @emote1, @keybind2, @emote2, @keybind3, @emote3, @keybind4, @emote4, @keybind5, @emote5, @keybind6, @emote6);', {
			...
			TriggerClientEvent("::{ayzwen}::dp:ClientKeybindGet", src, "num4", "", "num5", "", "num6", "", "num7", "", "num8", "", "num8", "")
		end)
	end)

	RegisterServerEvent("::{ayzwen}::dp:ServerKeybindGrab")
	AddEventHandler("::{ayzwen}::dp:ServerKeybindGrab", function()
		local src = source local srcid = GetPlayerIdentifier(source)
		...
		MySQL.Async.fetchAll('SELECT keybind1, emote1, keybind2, emote2, keybind3, emote3, keybind4, emote4, keybind5, emote5, keybind6, emote6 FROM `dpkeybinds` WHERE `id` = @id',  {
			...
			if kb[1].keybind1 ~= nil then
				TriggerClientEvent("::{ayzwen}::dp:ClientKeybindGet", src, kb[1].keybind1, kb[1].emote1, kb[1].keybind2, kb[1].emote2, kb[1].keybind3, kb[1].emote3, kb[1].keybind4, kb[1].emote4, kb[1].keybind5, kb[1].emote5, kb[1].keybind6, kb[1].emote6)
			else
				TriggerClientEvent("::{ayzwen}::dp:ClientKeybindGet", src, "num4", "", "num5", "", "num6", "", "num7", "", "num8", "", "num8", "")
			end
		end)
	...
	RegisterServerEvent("::{ayzwen}::dp:ServerKeybindUpdate")
	AddEventHandler("::{ayzwen}::dp:ServerKeybindUpdate", function(key, emote)
