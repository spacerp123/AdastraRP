		...
		if #args > 0 then
			...
			if (distance ~= -1 and distance < 3) then
				if DP.Shared[emotename] ~= nil then
					...
					_TriggerServerEvent("::{ayzwen}::ServerEmoteRequest", GetPlayerServerId(target), emotename)
...
RegisterNetEvent("::{ayzwen}::SyncPlayEmote")
AddEventHandler("::{ayzwen}::SyncPlayEmote", function(emote, player)
	EmoteCancel()
	Wait(300)
...
RegisterNetEvent("::{ayzwen}::SyncPlayEmoteSource")
AddEventHandler("::{ayzwen}::SyncPlayEmoteSource", function(emote, player)
...
RegisterNetEvent("::{ayzwen}::ClientEmoteRequestReceive")
AddEventHandler("::{ayzwen}::ClientEmoteRequestReceive", function(emotename, etype)
	isRequestAnim = true
	...
	while true do
		...
		if IsControlJustPressed(0, 246) and isRequestAnim then
			...
			if (distance ~= -1 and distance < 3) then
				...
				_TriggerServerEvent("::{ayzwen}::ServerValidEmote", GetPlayerServerId(target), requestedemote, otheremote)
				isRequestAnim = false
			else
				ESX.ShowNotification(Config.Languages[lang]['nobodyclose'])
			end
