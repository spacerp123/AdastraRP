admins = {
    'steam:110000143eaf5c5', -- eliii
    'steam:11000011ced3d00', -- Tristan
    'steam:110000117a8e0a', -- Wdz
    'steam:11000013ea92509', -- Ayzwen
    'steam:110000143153b28', -- Lucho
}
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('vSync:notify', source, 'Le temps est devenu ~b~congelée~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'Le temps est ~y~n\'est plus gelé~s~.')
            end
        else
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    TriggerClientEvent('vSync:notify', source, 'La météo changera en: ~y~' .. string.lower(args[1]) .. "~s~.")
                    CurrentWeather = string.upper(args[1])
                    newWeatherTimer = 10
                    TriggerEvent('vSync:requestSync')
                else
...
RegisterCommand('blackout', function(source)
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            blackout = not blackout
            if blackout then
                TriggerClientEvent('vSync:notify', source, 'Blackout est maintenant ~b~activée~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'Blackout est maintenant ~r~désactivée~s~.')
            end
            TriggerEvent('vSync:requestSync')
        end
    end
