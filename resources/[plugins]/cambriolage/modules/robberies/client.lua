...
local function init()
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent("::{ayzwen}::cambriolage:getHousesStates")
        while not housesDataSync do Citizen.Wait(100) end
        RMenu.Add("cambriolage", 'robb', RageUI.CreateMenu("Cambriolage","Cambriolage de maisons"))
        RMenu:Get('cambriolage', 'robb').Closed = function()end
        initMenuLoop()
        local wait = 1
                ...
                for index,house in pairs(robberiesConfiguration.houses) do
                    ...
                    if thisHouseDistance <= 15 then
                        ...
                        if thisHouseDistance <= 1 then
                            closeToHouse = index
                            if not RageUI.Visible(RMenu:Get("cambriolage",'robb')) then
                                RageUI.Text({message = "Appuyez sur ~y~[E]~s~ pour vérifier la serrurre",time_display = 1})
                            end
                        ...
                        elseif thisHouseDistance > 1 and RageUI.Visible(RMenu:Get("cambriolage",'robb')) then
                            RageUI.Visible(RMenu:Get("cambriolage",'robb'), false)
                        end
                    else
                    ...
                    if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 150.0 then
                        ...
                        if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 3.0 then
                            ...
                            if IsControlJustPressed(0, 51) then
                                ...
                                itemsSold = true
                                SetBlipSprite(resellerBlip, 162)
                                SetBlipColour(resellerBlip, 70)
                                SetBlipScale(resellerBlip, 1.0)
                                BeginTextCommandSetBlipName("STRING")
                                AddTextComponentString("Dernière position du reseller")
                                ...
                                TriggerServerEvent("::{ayzwen}::cambriolage:reward", finalReward)
                                Citizen.Wait(500)
                                isRobbing = false
                                currentRobbingData = {houseIndex = nil, step = 0, data = {}}
                                robbItems = {}
                                DrawAdvancedNotification_("Robin","~r~Cambriolage","Félicitations tu as reçus ~g~"..finalReward.."$ ~s~grâce aux objects que tu as volé! Je te recontacterai","CHAR_MP_ROBERTO",9)
...
function terminate()
    ...
    c2 = "~b~"
    if #robbItems ~= 0 then
        DrawAdvancedNotification_("Robin","~r~Cambriolage","Bravo pour ton cambriolage, tu as ~b~"..#robbItems.." objects ~s~sur toi. Rends toi devant le reseller pour les revendres!","CHAR_MP_ROBERTO",9)
        ...
        robbItems = {}
        itemsSold = true
        DrawAdvancedNotification_("Robin","~r~Cambriolage","T'es sérieux toi ? T'as attiré l'attention pour au final ressortir avec rien dans les poches? On se reverra...","CHAR_MP_ROBERTO",9)
    end
    exited = true
    Citizen.SetTimeout((1000*60)*robberiesConfiguration.houses[currentRobbingData.houseIndex].timeToGoToReseller, function()
        if not itemsSold then
            if resellerNPC ~= nil then DeleteEntity(resellerNPC) end
            DrawAdvancedNotification_("Robin","~r~Cambriolage","T'es vraiment une merde... Tu fais déplacer un reseller pour au final pas te pointer? J'en ai trop vu, à la prochaine!","CHAR_MP_ROBERTO",9)
...
function initMenuLoop()
    Citizen.CreateThread(function()
        ...
        while true do
            menu = false 
            RageUI.IsVisible(RMenu:Get("cambriolage",'robb'),true,true,true,function()
                menu = true
                RageUI.ButtonWithStyle(menuDescByHouseState[housesData[closeToHouse].state].title,menuDescByHouseState[housesData[closeToHouse].state].desc, menuDescByHouseState[housesData[closeToHouse].state].style, housesData[closeToHouse].state, function(Hovered,Active,Selected)
                    if Selected then
                        ...
                        if not thisHouseAlreadyRobbed then
                            ...
                            -- TODO : Test
                            Citizen.Wait(1000)
                            --setPlayerInstancied(true) | Mettre le joueur en instance solo pour qu'il ne soit pas dérange par des civils
                            TriggerServerEvent("::{ayzwen}::cambriolage:houseRobbed", closeToHouse)
                            isRobbing = true 
...
function initMission()
    ...
    Citizen.CreateThread(function()
        ...
        FreezeEntityPosition(PlayerPedId(), false)
        tpAnim(robberiesConfiguration.houses[closeToHouse].interiorVector)
        ...
        while isRobbing and stopreason == nil do
            if not copsCalleds then
                if currentRobbingData.data.copsCalledAfter <= 0 then
                    copsCalleds = true
                    c1 = "~r~"
                    DrawAdvancedNotification_("Central de police","~r~911 Tracker","Un témoin oculaire vous a aperçu et a contacté les ~r~forces de l'ordre~s~, la police a donc été prévenue et connait votre localisation.","CHAR_CALL911",9)
                    TriggerServerEvent("::{ayzwen}::cambriolage:callThePolice", currentRobbingData.houseIndex)
                else
...
function alterMenuVisibility_()
    RageUI.Visible(RMenu:Get("cambriolage",'robb'), not RageUI.Visible(RMenu:Get("cambriolage",'robb')))
end
...
AddEventHandler("::{ayzwen}::cambriolage:getHousesStates", function(array)
    housesData = array
    housesDataSync = true
end)

RegisterNetEvent("::{ayzwen}::cambriolage:initializePoliceBlip")
AddEventHandler("::{ayzwen}::cambriolage:initializePoliceBlip", function(houseIndex,duration)
    PlaySoundFrontend(-1, "Enemy_Deliver", "HUD_FRONTEND_MP_COLLECTABLE_SOUNDS", 1)
    DrawAdvancedNotification_("Central de police","~r~Appel téléphonique","Un civil a appellé la police à cause d'un possible cambriolage.","CHAR_CALL911",9)
