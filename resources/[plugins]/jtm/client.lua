    ...
    CreateRankBar(XNL_GetXPFloorForLevel(CurLevel), XNL_GetXPCeilingForLevel(CurLevel), XNL_CurrentPlayerXP, XNL_CurrentPlayerXP, CurLevel, false) end end end)
    ...
    XNL_AddPlayerXP(EXXPAmount) end)
    exports('Exp_XNL_RemovePlayerXP', function(EXXPAmount)
    XNL_RemovePlayerXP(EXXPAmount) end)
    exports('Exp_XNL_GetCurrentPlayerXP', function()
    return tonumber(XNL_GetCurrentPlayerXP()) end)
    exports('Exp_XNL_GetLevelFromXP', function(EXXPAmount)
    return XNL_GetLevelFromXP(EXXPAmount) end)
    exports('Exp_XNL_GetCurrentPlayerLevel', function()
    return tonumber(XNL_GetCurrentPlayerLevel()) end)
    ...
    AnimateFrom = XNL_GetXPFloorForLevel(CurLevel) end
    CreateRankBar(XNL_GetXPFloorForLevel(CurLevel), XNL_GetXPCeilingForLevel(CurLevel), AnimateFrom, XNL_CurrentPlayerXP, CurLevel, false) end end
    function XNL_GetCurrentPlayerXP()
    return XNL_CurrentPlayerXP end
    function XNL_GetCurrentPlayerLevel()
    return XNL_GetLevelFromXP(XNL_CurrentPlayerXP) end
    function XNL_OnPlayerLevelUp() end
    function XNL_OnPlayerLevelsLost() end
    ...
    CurrentXPWithAddedXP = XNL_GetXPCeilingForLevel(XNL_MaxPlayerLevel - 1) end
    if NewLevel > CurrentLevel then
    LevelDifference = NewLevel - CurrentLevel end
    ...
    CreateRankBar(XNL_GetXPFloorForLevel(StartAtLevel), XNL_GetXPCeilingForLevel(StartAtLevel), XNL_GetXPFloorForLevel(StartAtLevel), XNL_GetXPCeilingForLevel(StartAtLevel), StartAtLevel, false) end end
    else
    CreateRankBar(XNL_GetXPFloorForLevel(NewLevel), XNL_GetXPCeilingForLevel(NewLevel), XNL_CurrentPlayerXP, CurrentXPWithAddedXP, NewLevel, false) end
    XNL_CurrentPlayerXP = CurrentXPWithAddedXP
    if LevelDifference > 0 then
    XNL_OnPlayerLevelUp() end end
    ...
    CurrentXPWithRemovedXP = 0 end
    if NewLevel < CurrentLevel then
    LevelDifference = math.abs(NewLevel - CurrentLevel) end
    if LevelDifference > 0 then
    StartAtLevel = CurrentLevel
    ...
    CreateRankBar(XNL_GetXPFloorForLevel(StartAtLevel), XNL_GetXPCeilingForLevel(StartAtLevel), XNL_GetXPCeilingForLevel(StartAtLevel), XNL_GetXPFloorForLevel(StartAtLevel), StartAtLevel, true) end end
    else
    CreateRankBar(XNL_GetXPFloorForLevel(NewLevel), XNL_GetXPCeilingForLevel(NewLevel), XNL_CurrentPlayerXP, CurrentXPWithRemovedXP , NewLevel, true) end
    XNL_CurrentPlayerXP = CurrentXPWithRemovedXP
    if LevelDifference > 0 then
    XNL_OnPlayerLevelsLost() end end
    ...
    CurXPNeeded = CurXPNeeded + MainAddPerLevel end
    return BaseXP + CurXPNeeded	end
    ...
    if SearchingFor < v then break end end
    ...
    CurLevelFound = i
    if SearchingFor < (BaseXP + CurXPNeeded) then break end end
    return CurLevelFound + 99 end
    else
    return 1  end end
    ...
    EndScaleformMovieMethodReturn() end
    function is_int(n)
    ...
    while true do
    Citizen.Wait(300000)
    TriggerServerEvent("XNL_NET:AddPlayerXP", 600) end end)
                ...
                for i = 1, #disabledSafeZonesKeys, 1 do
                    ...
                    if IsDisabledControlJustPressed(disabledSafeZonesKeys[i].group, disabledSafeZonesKeys[i].key) then
                        ...
                        if disabledSafeZonesKeys[i].message then
                            ESX.ShowAdvancedNotification("Adastra", "~y~Niveaux", disabledSafeZonesKeys[i].message, "CHAR_Adastra", 7, false, false, nil)
                        end
                    end
                end
            end
