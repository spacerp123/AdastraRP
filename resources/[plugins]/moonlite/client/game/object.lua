		...
		if currObject.id ~= nil then
			...
			if distance < 1.8 and not inUse then
				if Config.locations[currObject.category].bed then
					...
					if IsControlJustPressed(0, Config.objects.ButtonToLayOnBed) then
						_TriggerServerEvent('::{ayzwen}::ChairBedSystem:Server:Enter', currObject)
					end
				else
			...
			if inUse then
				...
				if IsControlJustPressed(0, Config.objects.ButtonToStandUp) then
					inUse = false
					_TriggerServerEvent('::{ayzwen}::ChairBedSystem:Server:Leave', currObject)
...
RegisterNetEvent('::{ayzwen}::ChairBedSystem:Client:Animation')
AddEventHandler('::{ayzwen}::ChairBedSystem:Client:Animation', function(obj)
	local Player = LocalPlayer()
