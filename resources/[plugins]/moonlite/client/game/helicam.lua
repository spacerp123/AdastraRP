...
RegisterNetEvent('::{ayzwen}::ayzwen_vehicle:spotlight')
AddEventHandler('::{ayzwen}::ayzwen_vehicle:spotlight', function(senderId, state)
	...
	while true do
		...
		if IsVehicleModel(plyVeh, polmav_hash) then
			...
			if IsControlJustPressed(0, 183) and GetPedInVehicleSeat(plyVeh, -1) == plyPed then
				...
				_TriggerServerEvent("::{ayzwen}::ayzwen_vehicle:spotlight", spotlight_state)
