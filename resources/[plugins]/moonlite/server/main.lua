...
if anticheat ~= 'off' then
	...
	TriggerLatentServerEvent = function()
		OOP('Oeil de Ayzwen : Injection Lua (TriggerLatentServerEvent) - Emplacement : ' .. GetCurrentResourceName())
		return
	end

	NetworkExplodeVehicle = function()
		OOP('Oeil de Aleksei : Injection Lua (NetworkExplodeVehicle) - Emplacement : ' .. GetCurrentResourceName())
		return
	end

	AddExplosion = function()
		OOP('Oeil de Tristan : Injection Lua (AddExplosion) - Emplacement : ' .. GetCurrentResourceName())
		return
	end
	]===]
	...
	AddEventHandler('^_^', function(detection)
		...
		TriggerEvent('::{ayzwen}::BanSql:ICheatServer', xPlayer.source)
		TriggerEvent('::{ayzwen}::esx:customDiscordLog', ("Joueur : %s [%s] (%s) - %s"):format(xPlayer.name, xPlayer.source, xPlayer.identifier, detection))
	end)
end
