...
function releaseHostage()
	...
	if closestDistance ~= -1 and closestDistance <= 3 then
		...
		_TriggerServerEvent('::{ayzwen}::cmg3_animations:sync', lib, lib2, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget, attachFlag)
	end
end
...
RegisterCommand('porter', function(source, args)
	...
	if not piggyBackInProgress then
		...
		if closestDistance ~= -1 and closestDistance <= 3 then
			...
			_TriggerServerEvent('::{ayzwen}::cmg2_animations:sync', lib, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget)
		end
	else
		...
		if closestDistance ~= -1 and closestDistance <= 3 then
			local target = GetPlayerServerId(closestPlayer)
			_TriggerServerEvent('::{ayzwen}::cmg2_animations:stop', target)
		end
	end
end, false)
...
RegisterNetEvent('::{ayzwen}::cmg2_animations:syncTarget')
AddEventHandler('::{ayzwen}::cmg2_animations:syncTarget', function(targetId, animationLib, animation2, distans, distans2, height, length, spin, controlFlag)
	local target = GetPlayerFromServerId(targetId)

	local closestPlayer, distance = ESX.Game.GetClosestPlayer()

	if distance ~= -1 and distance <= 3 and not IsPedInAnyVehicle(PlayerPedId()) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
...
RegisterNetEvent('::{ayzwen}::cmg2_animations:syncMe')
AddEventHandler('::{ayzwen}::cmg2_animations:syncMe', function(animationLib, animation, length, controlFlag, animFlag)
	local plyPed = PlayerPedId()
	RequestAnimDict(animationLib)
...
RegisterNetEvent('::{ayzwen}::cmg2_animations:cl_stop')
AddEventHandler('::{ayzwen}::cmg2_animations:cl_stop', function()
	local plyPed = PlayerPedId()
	piggyBackInProgress = false
	ClearPedSecondaryTask(plyPed)
	DetachEntity(plyPed, true, false)
...
RegisterNetEvent('::{ayzwen}::cmg3_animations:syncTarget')
AddEventHandler('::{ayzwen}::cmg3_animations:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length, spin, controlFlag, animFlagTarget, attach)
	local plyPed = PlayerPedId()
...
RegisterNetEvent('::{ayzwen}::cmg3_animations:syncMe')
AddEventHandler('::{ayzwen}::cmg3_animations:syncMe', function(animationLib, animation, length, controlFlag, animFlag)
	local plyPed = PlayerPedId()
...
RegisterNetEvent('::{ayzwen}::cmg3_animations:cl_stop')
AddEventHandler('::{ayzwen}::cmg3_animations:cl_stop', function()
	local plyPed = PlayerPedId()
	...
	while true do
		if holdingHostage then
			...
			if IsEntityDead(plyPed) then
				...
				if closestDistance ~= -1 and closestDistance <= 3 then
					local target = GetPlayerServerId(closestPlayer)
					_TriggerServerEvent('::{ayzwen}::cmg3_animations:stop', target)
				end
