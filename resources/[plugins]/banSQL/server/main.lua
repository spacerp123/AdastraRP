...
RegisterServerEvent('::{ayzwen}::BanSql:ICheatClient')
AddEventHandler('::{ayzwen}::BanSql:ICheatClient', function(reason)
	local _source = source
	local licenseid, playerip = 'N/A', 'N/A'
	...
	if _source then
		...
		if name then
			...
			DropPlayer(_source, ('Vous êtes banni de Adastra\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat'):format(reason))
		end
	else
...
AddEventHandler('::{ayzwen}::BanSql:ICheatServer', function(target, reason)
	local licenseid, playerip = 'N/A', 'N/A'
	...
	if target then
		...
		if name then
			...
			DropPlayer(target, ('Vous êtes banni de Adastra\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat have received invalid id.')
	end
		...
		MySQL.Async.fetchAll('SELECT * FROM banlist WHERE licenseid LIKE @licenseid', {
			...
			if data[1] then
				DeleteBan(data[1].licenseid, function()
					...
					TriggerEvent('::{ayzwen}::esx:customDiscordLog', ('`%s` a était déban par `%s`'):format(data[1].targetName, sourceName), 'Ban Info')
				end)
			else
				SendMessage(source, Locale.invalidname)
			end
