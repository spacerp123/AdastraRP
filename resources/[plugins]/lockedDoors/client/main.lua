...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('::{ayzwen}::lockedDoors:getDoorInfo', function(doorInfo, doorCount)
				...
				if PlayerData.job ~= nil or PlayerData.job2 ~= nil then
					...
					if IsControlJustReleased(0, 38) then
						if (PlayerData.job.name == theDoor.job) or (PlayerData.job2.name == theDoor.job) then
							...
							_TriggerServerEvent('::{ayzwen}::lockedDoors:updateState', i, theDoor.locked, theDoor.job)
						else
							ESX.ShowAdvancedNotification('Adastra', '~y~Clefs', "Vous n'avez pas les clés de cette porte.", 'CHAR_Adastra', 7)
						end
					end
				end
...
RegisterNetEvent('::{ayzwen}::lockedDoors:setState')
AddEventHandler('::{ayzwen}::lockedDoors:setState', function(doorID, state)
